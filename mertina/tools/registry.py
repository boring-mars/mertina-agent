...
from mertina.constants import hermes_home_key
...
def _save_discovery_cache(cache: dict[str, list]) -> None:
    ...
    if path is None:
        return
    try:
        from mertina.constants import mkdir_under_hermes_home
        from mertina.utils import atomic_json_write  # stdlib+yaml only; no cycle
...
_check_fn_ever_good: set[tuple[Callable, str | None]] = (
    set()
)  # probes that admitted tools this process
_check_fn_core_drop_warned: set[tuple[Callable, str | None]] = (
    set()
)  # once-per-process WARNING gate
...
def check_fn_cache_scope() -> str | None:
    ...
    try:
        ...
        if all(str(get_session_env(k, "") or "").strip() for k in _BROWSER_IDENTITY_KEYS):
            ...
            from mertina.gateway.browser_control_broker import browser_control_enabled
        ...
        pass
    try:
        from mertina.agent.secret_scope import serves_routed_profile
...
def _run_check_fn_uncached(fn: Callable) -> bool:
    ...
    from mertina.agent.secret_scope import UnscopedSecretError, current_secret_scope
...
def _check_fn_cached(fn: Callable) -> bool:
    ...
    with _check_fn_cache_lock:
        ...
        if (
            core_dropped
            and cache_key in _check_fn_ever_good
            and cache_key not in _check_fn_core_drop_warned
        ):
...
def _core_tools_gated_by(fn: Callable) -> set[str]:
    ...
    try:
        from mertina.toolsets import _HERMES_CORE_TOOLS
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        self._plugin_override_policy: dict[tuple[str | None, str], _PluginOverridePolicy] = {}
    ...
    def _slot(self, scope: str | None, *, create: bool = False) -> dict[str, ToolEntry]:
    ...
    def _toolset_entries(self, toolset: str, scope: str | None) -> list[ToolEntry]:
    ...
    def get_entry(self, name: str, *, scope: str | None = None) -> ToolEntry | None:
        """Active profile's entry by name, falling back to global."""
        with self._lock:
            return self._merged_tools(scope).get(name)

    def snapshot_registration(self, name: str, *, scope: str | None = None) -> ToolEntry | None:
    ...
    def _plugin_override_allowed(self, scope: str | None, module_namespace: str) -> bool:
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> str | None:
    ...
    def register(
        ...
        with self._lock:
            ...
            plugin_override_denied = owner is not None and not self._plugin_override_allowed(
                scope, owner
            )
            shadows_global = (
                owner is not None
    ...
    def deregister(self, name: str, *, scope: str | None = None) -> None:
    ...
    def restore_registration(
        self, name: str, current: ToolEntry, previous: ToolEntry | None, *, scope: str | None = None
    ) -> bool:
    ...
    def dispatch(self, name: str, args: dict, *, scope: str | None = None, **kwargs) -> str | dict:
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from mertina.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
