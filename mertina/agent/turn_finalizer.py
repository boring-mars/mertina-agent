...
from mertina.agent.served_model import result_model_fields
from mertina.agent.turn_failure_copy import exit_reason_failure, stamp_failure
...
def _record_kanban_budget_exhausted(
    ...
    """
    try:
        from mertina.cli import kanban_db_connect as _kbc
        from mertina.cli import kanban_db_dispatch as _kbd
...
def _invoke_hook_safely(name: str, logger: logging.Logger, **kwargs) -> list:
    ...
    try:
        from mertina.cli.lifecycle import invoke_hook
...
def _close_transcript_tail(
    agent, messages, final_response, interrupted, _recovered_from_stream
) -> None:
    ...
    try:
        ...
        if (
            ...
            if _explanation:
                ...
                final_response = (
                    _explanation if _is_empty_terminal else _stripped + "\n\n" + _explanation
                )
...
def finalize_turn(
    ...
    *,
    ...
    from mertina.agent.turn_context import start_deferred_title_upgrade

    _guarded_cleanup(
        "start_deferred_title_upgrade",
        lambda: start_deferred_title_upgrade(agent),
        _cleanup_errors,
        logger,
    )
    ...
    try:
        from mertina.agent.conversation_loop import _notify_context_engine_turn_complete
