...
from mertina.agent import prompt_builder as _pb
...
from mertina.agent.runtime_cwd import resolve_context_cwd
from mertina.constants import get_default_hermes_root, get_hermes_home
from mertina.utils import is_truthy_value
...
def _resolve_platform_hint(agent: Any, platform_key: str, default_hint: str) -> str:
    ...
    replace_text, append_text = (
        v.strip() if isinstance(v, str) else "" for v in (spec.get("replace"), spec.get("append"))
    )
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    if (
        not hint
        or _TUI_EMBEDDED_PANE_CLARIFIER in hint
        or not is_truthy_value(os.getenv("HERMES_DESKTOP_TERMINAL"))
    ):
        return hint
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from mertina.cli.plugins import render_system_prompt_sections
            ...
            if rendered:
                logger.warning(
                    "Plugin system prompt sections failed to re-render (%s); keeping the previous frozen sections",
                    exc,
                )
            else:
...
def _restore_plugin_prompt_sections(prompt: str) -> tuple:
    ...
    from mertina.cli.plugins import (
...
def _plugin_section_blocks(sections: tuple, position: str) -> list[str]:
    from mertina.cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Path | None:
    ...
    """
    try:
        from mertina.constants import get_hermes_home_override
...
def _profile_name_for_home(home: Path) -> str:
    ...
    try:
        from mertina.constants import get_default_hermes_root
...
def _tool_guidance_block(agent: Any) -> str | None:
...
def _skills_prompt(agent: Any) -> str:
    ...
    avail_toolsets = {
        model_tools.get_toolset_for_tool(tool_name) for tool_name in agent.valid_tool_names
    } - {None, ""}
    try:
        from mertina.agent.coding_context import coding_compact_skill_categories
    ...
    if not getattr(agent, "_auto_load_skills_resolved", False):
        ...
        try:
            if not is_truthy_value(os.environ.get("HERMES_IGNORE_RULES")):
                from mertina.agent.skill_commands import build_auto_load_prompt
            ...
            logger.debug(
                "skills.auto_load: injection skipped", exc_info=True
            )  # config errors never block session start
...
def _bot_mode_parts(agent: Any) -> list[str]:
    ...
    try:
        ...
        _bot_section = (
            get_bot_mode_protocol_section(_agent_home(agent)) if _title == BOT_CHAT_TITLE else None
        )
        if _bot_section:
...
def _ambient_file_safety_profile_name() -> str:
    from mertina.agent.file_safety import _resolve_active_profile_name
...
def _active_profile_line(agent: Any) -> str:
    ...
    if active_profile == "default":
        ...
        _root_str = str(
            get_default_hermes_root() if _agent_home_path is not None else get_hermes_home()
        )
        return (
...
def _default_platform_hint(platform_key: str) -> str:
    ...
    if not hint and platform_key:
        try:
            from mertina.gateway.platform_registry import platform_registry
...
def platform_hint(agent: Any) -> str:
    ...
    _effective_hint = _resolve_platform_hint(
        agent, platform_key, _default_platform_hint(platform_key)
    )
...
def _telegram_rich_messages_enabled() -> bool:
    ...
    try:
        ...
        _gw = (((_cfg.get("gateway") or {}).get("platforms") or {}).get("telegram") or {}).get(
            "extra"
        )
        _top = ((_cfg.get("platforms") or {}).get("telegram") or {}).get("extra")
        merged = {
            **(_gw if isinstance(_gw, dict) else {}),
            **(_top if isinstance(_top, dict) else {}),
        }
...
def _memory_parts(agent: Any) -> list[str]:
    ...
    if agent._memory_store:
        for enabled, kind in (
            ...
            (agent._user_profile_enabled, "user"),
        ):
    ...
    if agent._memory_manager:
        try:
            from mertina.agent.memory_manager import memory_provider_tools_exposed as _mem_exposed
        except Exception:
            _mem_exposed = None
...
def _identity_parts(agent: Any, ctx_len: int | None) -> tuple[list[str], bool]:
    ...
    _soul_content = (
        _pb.load_soul_md(ctx_len, home_override=_agent_home(agent)) if wants_soul else None
    )
...
def _coding_parts(agent: Any) -> tuple[list[str], list[str], list[str]]:
    ...
    """
    try:
        from mertina.agent.coding_context import coding_system_prompt_parts
...
def _post_workspace_parts(agent: Any) -> list[str]:
    ...
    if getattr(agent, "_environment_probe", True):
        try:
            from mertina.tools.env_probe import get_environment_probe_line
...
def _context_files_part(agent: Any, ctx_len: int | None, soul_loaded: bool) -> list[str]:
    ...
    return [
        _pb.build_context_files_prompt(
...
def build_system_prompt_parts(agent: Any, system_message: str | None = None) -> dict[str, str]:
    ...
    if environment_hints:
        ...
        environment_hints = environment_hints.replace(
            _pb.RUNTIME_ENVIRONMENT_HEADING, "> " + _pb.RUNTIME_ENVIRONMENT_HEADING
        )
