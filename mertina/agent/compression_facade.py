# Ported from hermes-agent agent/compression_facade.py @ fbc4ea8b96
...
def _timeout_fallback_prompt(agent, system_message: str) -> str:
    ...
    try:
        ...
        logger.debug(
            "compress_context timeout fallback prompt rebuild failed; using raw system_message",
            exc_info=True,
        )
...
def _report_compression_timeout(
    ...
    *,
    ...
    from mertina.agent.conversation_compression import mark_context_compression_timed_out
        ...
        try:
            if total_exhausted:
                record(
                    ...
                    failure_kind="ceiling_exhausted",
                )
            else:
                record(
                    "host compress_context timeout (no summary progress)", failure_kind="stalled"
                )
        except Exception:
...
def _sync_persisted_markers(target_messages, source_messages) -> None:
    ...
    from mertina.agent.conversation_compression import _stamp_scoped_twins
...
def _run_under_progress_timeout(
    ...
    *,
    ...
    _same_turn_fallback_worker = functools.partial(
        _snapshot_worker, same_turn_fallback_recovery=True
    )
    ...
    return run_compress_context_with_progress_timeout(
        ...
        fallback_worker=_same_turn_fallback_worker,
...
class CompressionFacadeMixin:
    ...
    def _compress_context(
        ...
        *,
        ...
        from mertina.agent.portal_tags import (
            get_affinity_scope,
        ...
        from mertina.agent.prompt_cache_scope import declared_conversation_scope_safe
