...
from mertina.agent.lazy_forward import lazy_attr as _lazy_attr
from mertina.cli.timeouts import get_provider_request_timeout
from mertina.utils import base_url_host_matches, env_float
...
def _routermint_headers() -> dict:
    ...
    from mertina.cli import __version__ as _HERMES_VERSION
...
_ROUTE_DEFAULT_HEADERS = (
    ...
    (
        ...
        lambda self, url: dict(_lazy_attr("agent.auxiliary_client", "_AI_GATEWAY_HEADERS")),
    ),
    (
        "integrate.api.nvidia.com",
        lambda self, url: _lazy_attr("agent.auxiliary_client", "build_nvidia_nim_headers")(url),
    ),
    ("api.routermint.com", lambda self, url: _routermint_headers()),
    (
        "githubcopilot.com",
        lambda self, url: _lazy_attr("hermes_cli.models", "copilot_default_headers")(),
    ),
    (
        "api.kimi.com",
        lambda self, url: dict(_lazy_attr("agent.auxiliary_client", "_AI_GATEWAY_HEADERS")),
    ...
    (
        "chatgpt.com",
        lambda self, url: _lazy_attr("agent.codex_headers", "codex_cloudflare_headers")(
...
def _swap_fallback_clients(
    agent, fb_client, fb_provider: str, fb_model: str, fb_base_url: str, fb_api_mode: str
) -> None:
    ...
    if fb_api_mode == "anthropic_messages":
        ...
        from mertina.agent.anthropic_credentials import (
        ...
        agent._anthropic_client = build_anthropic_client(
            effective_key, fb_base_url, timeout=timeout
        )
    ...
    fb_headers = getattr(fb_client, "_custom_headers", None) or getattr(
...
class ClientLifecycleMixin:
    def _close_task_resources(self, task_id: str) -> None:
        ...
        from mertina.run_agent import _quietly, cleanup_browser, cleanup_vm

        def kill_processes() -> None:
            from mertina.tools.process_registry import process_registry
        ...
        def release_computer_use() -> None:
            from mertina.tools.computer_use.tool import release_computer_use_session
        ...
        def forget_file_state() -> None:
            ...
            from mertina.tools.file_tools import clear_file_ops_cache
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        from mertina.agent.process_bootstrap import build_keepalive_http_client
    ...
    _recover_with_credential_pool = _forward(
    ...
    def _drain_transports_after_abandonment(self, *, reason: str) -> int:
        ...
        try:
            request_interrupt = getattr(
                getattr(self, "_codex_session", None), "request_interrupt", None
            )
    ...
    def _build_anthropic_client_for_key(self, key: tuple) -> Any:
        from mertina.agent.anthropic_adapter import (
    ...
    def _create_request_anthropic_client(self, *, reason: str) -> Any:
        ...
        logger.debug(
            "Anthropic request client created (%s, shared=False) %s",
            reason,
            self._anthropic_log_context(),
        )
    ...
    def _close_request_anthropic_client(self, client: Any, *, reason: str) -> None:
        ...
        try:
            ...
            client.close()
            logger.info(
                "Anthropic client closed (%s, shared=False) %s",
                reason,
                self._anthropic_log_context(),
            )
        except Exception as exc:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from mertina.cli import auth as _auth

            resolve = (
                _auth.resolve_codex_runtime_credentials
        ...
        if singleton_key and old_key and singleton_key != old_key:
            logger.debug(
                ...
                "silent account swap. Reactive credential rotation should go through the pool.",
                self.provider,
            )
            return False
        try:
        ...
        if old_key and api_key.strip() == old_key:
            logger.debug(
                "%s credential refresh returned the same token; refresh likely failed silently",
                self.provider,
            )
            return False
        return self._adopt_openai_credentials(
            api_key, base_url, reason=f"{self.provider}_credential_refresh"
        )

    def _try_refresh_nous_client_credentials(
        self, *, force: bool = True, require_account: str | None = None
    ) -> bool:
        ...
        if self.provider != "nous" or self.api_mode not in (
            ...
            return False
        try:
            from mertina.cli.auth import resolve_nous_runtime_credentials
        ...
        if not _valid_credential_pair(api_key, base_url):
            return False
        if str(api_key).strip() == str(self.api_key or "").strip() and str(base_url).strip().rstrip(
            "/"
        ...
        if require_account is not None:
            try:
                from mertina.cli.auth_constants import _decode_jwt_claims
    ...
    def _adopt_nous_key_before_expiry(self) -> bool:
        ...
        if (
            ...
            or exp - time.time() > self._NOUS_KEY_ADOPT_SKEW_S
        ):
            return False
        return self._try_refresh_nous_client_credentials(force=False, require_account=str(account))

    def _resolve_env_credentials(self) -> tuple | None:
        ...
        """
        try:
            from mertina.agent.credential_pool import get_env_prefer_dotenv
        ...
        if (
            ...
            api_key = next(
                (
                    k
                    for k in (get_env_prefer_dotenv(v).strip() for v in pconfig.api_key_env_vars)
                    if k
                ),
                "",
            )
            ...
            if self.provider == "actual":
                ...
                from mertina.cli.runtime_provider import (
                ...
                resolver = (
                    _auth._resolve_kimi_base_url
                    if self.provider == "kimi-coding"
                    else _auth._resolve_zai_base_url
                )
            ...
            try:
                from mertina.cli.runtime_provider import _get_named_custom_provider
            except ImportError:
                return None
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        ...
        self._env_creds_seen = (base_url, api_key)
        logger.info(
            "Applied updated .env credentials for %s: endpoint %s", self.provider, self.base_url
        )
        return True
    ...
    def _try_refresh_vertex_client_credentials(self) -> bool:
        ...
        if self.api_mode != "chat_completions" or self.provider != "vertex":
            return False
        try:
            from mertina.agent.vertex_adapter import get_vertex_config
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from mertina.cli.copilot_auth import (
            ...
            return False
        new_token, enterprise_base_url = (
            (new_token.strip() if isinstance(new_token, str) else ""),
            None,
        )
        if not new_token:
            return False
        ...
        ok = self._apply_copilot_token(
            new_token, enterprise_base_url, reason="copilot_credential_refresh"
        )
        if ok:
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            ...
            return False
        self._anthropic_api_key, self._is_anthropic_oauth = (
            new_token,
            self._anthropic_oauth_flag(new_token),
        )
        return True

    # ------------------------------------------------------------------ route-derived client config
    def _apply_client_headers_for_base_url(
        self, base_url: str, *, apply_user_headers: bool = True
    ) -> None:
        for host, build in _ROUTE_DEFAULT_HEADERS:
            ...
            with suppress(Exception):
                ...
                if (
                    profile
                    and profile.default_headers
                    and (profile_headers := dict(profile.default_headers))
                ):
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from mertina.cli.config import apply_custom_provider_extra_headers_to_client_kwargs
    ...
    def _swap_credential(self, entry) -> bool:
        ...
        from mertina.cli.anon_auth import route_can_serve_model

        if not route_can_serve_model(
            getattr(self, "provider", None), stripped_base, getattr(self, "model", None)
        ):
            logger.info(
    ...
    def _reapply_route_client_config(self, *, route_changed: bool) -> None:
        ...
        try:
            ...
            logger.debug(
                "custom-provider TLS resolution skipped on credential rotation", exc_info=True
            )
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        from mertina.agent.anthropic_adapter import create_anthropic_message
    ...
    def _rebuild_anthropic_client(self) -> None:
        ...
        self._anthropic_client = self._build_anthropic_client_for_key(
            self._request_anthropic_client_key()
        )
