...
from mertina.agent.turn_failure_copy import site_copy, stamp_failure
...
def perform_api_call(
    ...
    *,
    ...
    from mertina.cli.middleware import run_llm_execution_middleware
...
def nous_rate_limit_guard(
    ...
    *,
    ...
    if agent.provider == "nous":
        ...
        try:
            from mertina.cli.anon_auth import apply_model_switch

            apply_model_switch(agent)
        except Exception:
            pass
        try:
            from mertina.agent.nous_rate_guard import format_remaining as _fmt_nous_remaining
            from mertina.agent.nous_rate_guard import nous_rate_limit_remaining
            from mertina.cli import anon_auth
                            ...
                            **(
                                {
                                    "free_tier": {
                                        "kind": "rate_limited",
                                        "message": anon_auth.FREE_TIER_RATE_LIMIT_CARD.format(
                                            reset=anon_auth.friendly_wait(_nous_remaining)
                                        ),
                                    }
                                }
