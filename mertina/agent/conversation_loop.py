...
from mertina.agent.codex_responses_adapter import _summarize_user_message_for_log
from mertina.agent.fast_mode import begin_turn as begin_fast_mode_turn
from mertina.agent.message_metadata import append_message
from mertina.agent.message_sanitization import _repair_tool_call_arguments, _sanitize_surrogates
from mertina.agent.model_metadata import MINIMUM_CONTEXT_LENGTH, _estimate_tools_tokens_rough
from mertina.agent.process_bootstrap import _install_safe_stdio
from mertina.agent.prompt_caching import (
...
from mertina.agent.runtime_cwd import resolve_agent_cwd
from mertina.agent.surface_switch import (
    identity_line_value,
...
from mertina.agent.turn_context import PreflightCompressionTimedOut, build_turn_context
from mertina.agent.turn_failure_copy import failed_turn_notice, site_copy
...
from mertina.agent.turn_preflight_gate import run_preflight_gate
...
from mertina.logging import set_session_context
from mertina.tools.skill_provenance import set_current_write_origin
...
def _midturn_request_pressure_tokens(
    ...
    """
    try:
        from mertina.agent.codex_responses_adapter import estimate_native_responses_preflight_tokens
...
def _maybe_inject_run_budget_wrapup(agent: Any, messages: list[dict[str, Any]]) -> bool:
    ...
    if (
        ...
        return False
    from mertina.agent.context_compressor import _DB_PERSISTED_MARKER
...
def _should_skip_model_call_for_reference_handoff(
    ...
    from mertina.agent.context_compressor import reference_handoff_would_drive_next_model_call
...
def _maybe_grow_local_window(agent: Any, compressor: Any, request_tokens: int) -> int | None:
    ...
    if provider not in ("llamacpp", "llama.cpp", "llama-cpp", "custom") or not (
        ...
        return None
    try:
        from mertina.cli.local_runtime.growth import maybe_grow_window
...
def _nous_entitlement_message(capability: str) -> str:
    try:
        from mertina.cli.nous_account import (
...
def _billing_or_entitlement_message(
    *, capability: str, provider: str, base_url: str, model: str, unverified: bool = False
    ...
    try:
        from mertina.agent.billing_links import build_billing_block
...
def _bot_chat_prompt_stale(agent, stored_prompt: str) -> bool:
    ...
    re-fire."""
    try:
        from mertina.tools.bot_mode_probe import (
            BOT_CHAT_TITLE,
        ...
        home = None
        try:
            from mertina.agent.system_prompt import _agent_home
...
def _persist_system_prompt(agent, failure_message: str, *, persist_tools: bool = False) -> None:
    ...
    try:
        ...
        if persist_tools:
            from mertina.tools.mcp_tool_agent import persist_agent_tool_names
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if conversation_history and agent._session_db:
        try:
            ...
            if session_row is not None:
                ...
                stored_state = (
                    "null" if raw_prompt is None else ("empty" if raw_prompt == "" else "present")
                )
    ...
    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        if _bot_chat_prompt_stale(agent, stored_prompt):
            ...
            try:
                from mertina.agent.prompt_builder import clear_skills_system_prompt_cache
        ...
        try:
            ...
            if saved_tools:
                from mertina.tools.mcp_tool_agent import agent_tool_names, restore_agent_tool_prefix
    ...
    if not getattr(agent, "_persist_disabled", False):
        try:
            from mertina.cli.lifecycle import invoke_hook as _invoke_hook
    ...
    try:
        from mertina.agent.credits_tracker import seed_credits_at_session_start
...
def _get_continuation_prompt(is_partial_stub: bool, dropped_tools: list[str] | None = None) -> str:
    ...
    return (
        _LENGTH_CONTINUATION_NETWORK_STUB if is_partial_stub else _LENGTH_CONTINUATION_OUTPUT_LIMIT
    )
...
def _rewrite_system_content_blocks(system_message: dict, effective: str) -> bool:
    ...
    if (
        ...
        or not content
        or not all(isinstance(part, dict) and part.get("type") == "text" for part in content)
    ):
        return False
    if len(content) == 1:
...
def _ensure_cached_system_prompt_static(agent, system_message=None) -> None:
    ...
    from mertina.agent.system_prompt import reconstruct_static_prefix
...
def _peel_moa_guidance(messages: list[dict[str, Any]], guidance: Any) -> list[dict[str, Any]]:
    ...
    from mertina.agent.moa_loop import peel_reference_guidance
...
def _redecorate_prompt_cache_for_provider(
    ...
    *,
    ...
    # AIAgent, and a default would mask a real init bug as silent cache-off.
        ...
        from mertina.agent.prompt_caching import envelope_tool_part_cache_markers_supported
...
def _engine_overrides_hook(engine: Any, name: str) -> bool:
    ...
    if engine is None or not callable(hook):
        return False
    try:
        from mertina.agent.context_engine import ContextEngine as _CE
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from mertina.cli.moa_config import decode_moa_turn
...
def _preflight_timeout_result(agent, exc, conversation_history) -> dict[str, Any]:
    ...
    from mertina.agent.agent_runtime_helpers import note_turn_persisted
...
def _run_conversation_turn(
    ...
    if agent.api_mode == "codex_app_server":
        ...
        from mertina.agent.turn_recovery import activate_codex_app_server_fallback
    ...
    if s._compression_timeout_exhausted:
        ...
        result.update(
            error=_COMPRESSION_TIMEOUT_FINAL_RESPONSE, partial=True, compression_exhausted=True
        )
    return result
...
def run_conversation(
    ...
    """
    from mertina.agent.turn_context import export_current_turn_boundary
    from mertina.tools.vision_tools_history_budget import native_turn_images
...
def _close_durable_failed_turn(agent, result: Any) -> None:
    ...
    try:
        ...
        if (
            ...
            or db.latest_conversation_role(session_id) != "user"
        ):
            return
        ...
        turn_messages = (
            messages[start:] if isinstance(start, int) and 0 <= start < len(messages) else messages
        )
        append_message(
            messages, {"role": "assistant", "content": failed_turn_notice(turn_messages)}
        )
...
_PLUGIN_COMPAT_LAZY = {
    "COMPRESSION_RETRY_CONTEXT_REDUCED_STATUS_TEMPLATE": (
        "mertina.agent.conversation_compression",
        "COMPRESSION_RETRY_CONTEXT_REDUCED_STATUS_TEMPLATE",
    ),
    ...
    "KawaiiSpinner": ("mertina.agent.display", "KawaiiSpinner"),
    "PARTIAL_STREAM_STUB_ID": ("mertina.constants", "PARTIAL_STREAM_STUB_ID"),
    "PRE_API_COMPRESSION_STATUS_TEMPLATE": (
        "mertina.agent.conversation_compression",
        "PRE_API_COMPRESSION_STATUS_TEMPLATE",
    ),
    "adaptive_rate_limit_backoff": ("mertina.agent.retry_utils", "adaptive_rate_limit_backoff"),
    "anchored_context_tokens": ("mertina.agent.usage_anchor", "anchored_context_tokens"),
    "automatic_compaction_status_message": (
        "mertina.agent.context_engine",
        "automatic_compaction_status_message",
    ),
    "capture_usage_anchor": ("mertina.agent.usage_anchor", "capture_usage_anchor"),
    "classify_api_error": ("mertina.agent.error_classifier", "classify_api_error"),
    "close_interrupted_tool_sequence": (
        "mertina.agent.message_sanitization",
        "close_interrupted_tool_sequence",
    ),
    "coalesce_tool_call_id": ("mertina.agent.message_sanitization", "coalesce_tool_call_id"),
    "compose_user_api_content": ("mertina.agent.turn_context", "compose_user_api_content"),
    ...
    "env_var_enabled": ("mertina.utils", "env_var_enabled"),
    ...
    "estimate_usage_cost": ("mertina.agent.usage_pricing", "estimate_usage_cost"),
    "get_context_length_from_provider_error": (
        "mertina.agent.model_metadata",
        "get_context_length_from_provider_error",
    ),
    "has_incomplete_scratchpad": ("mertina.agent.trajectory", "has_incomplete_scratchpad"),
    "is_output_cap_error": ("mertina.agent.model_metadata", "is_output_cap_error"),
    "is_repetition_dominated": ("mertina.agent.repetition_guard", "is_repetition_dominated"),
    "is_zai_coding_overload_error": ("mertina.agent.retry_utils", "is_zai_coding_overload_error"),
    ...
    "reanchor_current_turn_user_idx": (
        "mertina.agent.turn_context",
        "reanchor_current_turn_user_idx",
    ),
    "save_context_length": ("mertina.agent.model_metadata", "save_context_length"),
    "serialized_messages_bytes": (
        "mertina.agent.message_sanitization",
        "serialized_messages_bytes",
    ),
    "splice_provider_projection": (
        "mertina.agent.provider_projection",
        "splice_provider_projection",
    ),
    "zai_coding_overload_retry_ceiling": (
        "mertina.agent.retry_utils",
        "zai_coding_overload_retry_ceiling",
    ),
}
