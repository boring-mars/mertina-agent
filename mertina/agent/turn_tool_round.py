...
from mertina.agent.turn_preflight import compress_after_tool_results
from mertina.agent.turn_tool_validation import validate_tool_calls
...
def run_tool_round(
    ...
    *,
    ...
    process-only state."""
    from mertina.agent.conversation_loop import _invalid_tool_name_error_content
    ...
    try:
        ...
        logger.warning(
            "Incremental tool-call persistence failed before execution (session=%s): %s",
...
def stage_tool_call_message(
    ...
    already showed."""
    from mertina.agent.conversation_loop import _STALE_MARKER_RE
    ...
    if assistant_message.tool_calls and _STALE_MARKER_RE.fullmatch(turn_content.strip()):
        logger.warning("Discarding bare tool-call marker from assistant content: %s", turn_content)
        turn_content = ""
