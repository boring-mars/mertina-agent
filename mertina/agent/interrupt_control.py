...
from mertina.agent.interrupt_compat import request_hard_interrupt
from mertina.tools.interrupt import request_yield as _request_yield
...
class InterruptControlMixin:
    ...
    def interrupt(
        ...
        *,
        ...
        tool_interrupt_reason = (
            (tool_reason or _REASON_HARD_STOP)
            if hard_cancel
    ...
    def clear_interrupt(
        self, *, preserve_redirect: bool = False, hard_cancel: bool = False
    ) -> bool:
        ...
        with _ic_lock(self, "_pending_redirect_lock"):
            if preserve_redirect and not _ic_slot(
