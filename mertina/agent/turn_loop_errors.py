...
from mertina.agent.turn_failure_copy import short_detail, site_copy
...
def handle_outer_loop_error(
    ...
    *,
    ...
    if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
        error_msg = (
            f"Interpreter is shutting down — cannot continue (API call #{api_call_count}): {e}"
        )
        try:
        ...
        final_response = site_copy(
            ...
            resume=f" (CLI: `hermes --resume {_sid}`)" if _sid else "",
        )
