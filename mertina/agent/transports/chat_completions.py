...
from mertina.agent.lmstudio_reasoning import resolve_lmstudio_effort
from mertina.agent.message_sanitization import normalize_finish_reason as _normalize_finish_reason
from mertina.agent.moonshot_schema import is_moonshot_model, sanitize_moonshot_tools
from mertina.agent.prompt_builder import DEVELOPER_ROLE_MODELS
from mertina.agent.reasoning_effort import (
    KIMI_K3_EFFORTS,
    KIMI_K3_OVERRIDES,
...
def _rename_tool_search_bridge_for_xai(
    ...
    """
    from mertina.agent.transports.codex import _alias_reserved_tools
...
def _add_prompt_cache_key(
    ...
    *,
    ...
    from mertina.agent.transports.codex import (
    ...
    containers = [
        c
        for c in (api_kwargs, api_kwargs.get("extra_body"))
        if isinstance(c, dict) and "prompt_cache_key" in c
    ]
    if containers:
        for c in containers:
...
def _raise_gemini_thinking_max_tokens(
    model: str, reasoning_config: dict | None, requested: Any
) -> Any:
    """Raise Gemini output caps that thinking tokens (billed against max_tokens) would otherwise exhaust."""
    thinking_config = _build_gemini_thinking_config(model, reasoning_config)
    if not thinking_config:
        return requested
    from mertina.agent.gemini_native_adapter import _effective_gemini_max_output_tokens
...
def _is_gemini_openai_compat_base_url(base_url: Any) -> bool:
    ...
    return (
        bool(normalized)
        and "generativelanguage.googleapis.com" in normalized
...
def _apply_max_tokens(
    ...
    for candidate in (params.get("ephemeral_max_output_tokens"), params.get("max_tokens")):
        if candidate is not None and max_tokens_fn:
            api_kwargs.update(
                max_tokens_fn(_raise_gemini_thinking_max_tokens(model, reasoning_config, candidate))
            )
            return
    if profile_max and max_tokens_fn:
        api_kwargs.update(
            max_tokens_fn(_raise_gemini_thinking_max_tokens(model, reasoning_config, profile_max))
        )


def _base_kwargs(
    model: str, sanitized: list, tools: Any, params: dict, profile: Any = None
) -> dict[str, Any]:
    ...
    if profile is not None:
        from mertina.providers.base import OMIT_TEMPERATURE
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(self, messages: list[dict[str, Any]], **kwargs) -> list[dict[str, Any]]:
        ...
        native_type = (
            getattr(kwargs.get("provider_profile"), "native_reasoning_details_type", None) or None
        )
        strip_reasoning_details = not (
            native_type or _route_replays_reasoning_details(kwargs.get("base_url"))
        )
        sanitized_pairs = [
            (m, _sanitize_message(m, strip_extra_content, strip_reasoning_details, native_type))
            for m in messages
        ]
    ...
    def build_kwargs(
        ...
        **params,
        ...
        sanitized = _swap_developer_role(
            sanitized, params.get("model_lower", (model or "").lower())
        )
        ...
        if is_lmstudio and supports_reasoning:
            _lm_effort = resolve_lmstudio_effort(
                reasoning_config, params.get("lmstudio_reasoning_options")
            )
        ...
        if str(params.get("provider_name") or "").strip().lower() == "gemini":
            ...
            if _is_gemini_openai_compat_base_url(base_url):
                ...
                if thinking_config:
                    ...
                    openai_compat_extra["google"] = {
                        **openai_compat_extra.get("google", {}),
    ...
    def _build_kwargs_from_profile(self, profile, model, sanitized, tools, params):
        ...
        if extra_body:
            ...
            try:
                from mertina.agent.gemini_native_adapter import is_native_gemini_base_url
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        finish_reason = (
            _normalize_finish_reason(str(_fr) if isinstance(_fr, int) else _fr) or "stop"
        )
