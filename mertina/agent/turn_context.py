...
from mertina.agent.conversation_compression import recover_rotated_compression_session
from mertina.agent.image_token_cost import bind_image_token_cost
from mertina.agent.iteration_budget import IterationBudget
from mertina.agent.memory_manager import build_memory_context_block
...
from mertina.agent.turn_author import parse_turn_author
from mertina.agent.usage_anchor import anchored_context_tokens, restore_usage_anchor
...
def _preflight_request_tokens(
    ...
    try:
        ...
        logger.debug(
            "native Responses preflight estimate unavailable; using generic transcript estimate",
            exc_info=True,
        )
...
def _agent_stale_thinking_on_wire(agent: Any) -> bool:
    ...
    try:
        from mertina.agent.message_sanitization import stale_thinking_reaches_wire
...
def _maybe_title_session_at_turn_start(agent: Any, messages: list[Any]) -> None:
    ...
    try:
        ...
        from mertina.agent.title_generator import maybe_auto_title
...
def start_deferred_title_upgrade(agent: Any) -> None:
    ...
    from mertina.agent.title_generator import start_title_upgrade
...
def reanchor_current_turn_user_idx(messages: list[Any], user_message: Any) -> int:
    ...
    """
    from mertina.agent.context_compressor import user_originated_turn_view
...
def export_current_turn_boundary(agent: Any, result: Any, user_message: Any) -> Any:
    ...
    if (
        ...
        or result.get("turn_exit_reason") == "context_compression_timeout"
    ):
        return result
...
def _should_idle_compact(
    *,
    ...
    if (
        not enabled
        or idle_after_seconds <= 0
        or idle_gap_seconds < idle_after_seconds
        or cooldown_active
    ):
        return False
...
def _publish_runtime_main(agent: Any) -> None:
    ...
    with suppress(Exception):
        from mertina.agent.auxiliary_client import set_runtime_main
        ...
        set_runtime_main(
            _str_attr(agent, "provider"),
            _str_attr(agent, "model"),
            **{
                k: _str_attr(agent, k)
                for k in (
                    "requested_provider",
...
def _refresh_mcp_tools_between_turns(agent: Any) -> None:
    ...
    try:
        ...
        if (
            ...
            and "mertina.tools.mcp_tool" in sys.modules
        ):
            from mertina.tools.mcp_tool_agent import refresh_agent_mcp_tools
            from mertina.tools.mcp_tool_discovery import has_registered_mcp_tools
...
def _bind_turn_identity(
    ...
    persist_user_platform_id: str | None,
) -> tuple[str, str]:
    ...
    agent._process_owner_task_ids = {
        *getattr(agent, "_process_owner_task_ids", ()),
        effective_task_id,
    }
    ...
    from mertina.agent.agent_runtime_helpers import note_turn_start
    ...
    if agent._user_turn_count == 0:
        ...
        if callable(note_checkpoint):
            try:
                ...
                if has_replayable_native_compaction_checkpoint(agent, conversation_history):
                    note_checkpoint()
            except Exception:
                logger.debug("restored native checkpoint hydration skipped", exc_info=True)
...
def _emit_reaction(agent: Any, original_user_message: Any) -> None:
    ...
    if reaction_callback is None:
        return
    with suppress(Exception):
        from mertina.agent.reactions import detect_reaction
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            from mertina.tools.hook_output_spill import get_spill_config as _spill_cfg
            from mertina.tools.hook_output_spill import spill_if_oversized as _spill_if_oversized
...
def _merge_gateway_notes(
    ...
    _turn_notes = "\n\n".join(
        part
        for part in (consume_gateway_turn_context_notes(agent), consume_surface_switch_note(agent))
        if part
    )
    if not _turn_notes:
...
def _stamp_api_content_sidecar(
    ...
    *,
    ...
    from mertina.agent.session_persistence import _persist_lock, durable_user_row_content
...
def build_turn_context(
    ...
    *,
    ...
    compression."""
    from mertina.agent.turn_context_compaction import run_turn_start_compaction
    ...
    from mertina.tools.skill_provenance import set_review_attended
    ...
    try:
        from mertina.tools.bot_mode_dm import ensure_message_agent_tool
    ...
    from mertina.agent.session_persistence import _PERSIST_AFTER_ADMISSION_INTERRUPT
...
def build_api_messages(
    ...
    *,
    ...
    replayed verbatim."""
    from mertina.agent.agent_runtime_helpers import fill_empty_non_final_wire_payload
    from mertina.agent.conversation_loop import _clone_message_for_send
    from mertina.agent.replay_cleanup import canonicalize_replay_history

    has_current = isinstance(current_turn_user_idx, int) and 0 <= current_turn_user_idx < len(
        messages
    )
    ...
    canonical_messages = (
        canonicalize_replay_history(messages[:split], now=turn_now) + messages[split:]
    )
