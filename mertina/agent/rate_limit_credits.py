# Ported from hermes-agent agent/rate_limit_credits.py @ fbc4ea8b96
...
class RateLimitCreditsMixin:
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from mertina.agent.rate_limit_tracker import parse_rate_limit_headers
    ...
    def _capture_nous_model_switch(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from mertina.cli.anon_auth import note_model_switch
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        try:
            from mertina.agent.credits_tracker import dev_fixture_credits_state
        ...
        try:
            from mertina.agent.credits_tracker import parse_credits_headers
    ...
    def _emit_credits_notices(self) -> None:
        ...
        if not self._credits_notices_enabled() or state is None:
            return
        try:
            from mertina.agent.credits_tracker import (
