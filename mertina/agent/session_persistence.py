# Ported from hermes-agent agent/session_persistence.py @ fbc4ea8b96
...
from mertina.agent.context_compressor import (
...
from mertina.agent.memory_manager import sanitize_context
from mertina.agent.tool_dispatch_helpers import _is_multimodal_tool_result, _multimodal_text_summary
from mertina.agent.trajectory import save_trajectory as _save_trajectory_to_file
from mertina.agent.transcript_repair import sync_flushed_message_markers

logger = logging.getLogger(
    "mertina.run_agent"
)  # origin module's name: log records / caplog filters unchanged
...
def _db_flush_seed_ids(agent) -> set:
    ...
    seed_ids = (
        getattr(agent, "_flushed_db_message_ids", None)
        if same_session and agent._last_flushed_db_idx != 0
        else None
    )
...
def _db_flush_collect(agent, messages: list[dict], conversation_history: list[dict] | None):
    ...
    for msg_idx in range(_db_flush_scan_start(agent, messages), len(messages)):
        ...
        if (
            ...
            or _is_ephemeral_scaffolding(msg)
            or msg.get(_DB_PERSISTED_MARKER)
        ):
            continue
        # Already durable (history copy or caller-seeded): stamp so future flushes skip it.
        if (id(msg) in history_ids or id(msg) in seed_ids) and not msg.get(
            _PERSIST_AFTER_ADMISSION_INTERRUPT
        ):
        ...
        if getattr(agent, "_mute_notification_reply", False):
            ...
            msg["display_metadata"] = {
                **(msg.get("display_metadata") or {}),
                "notification_category": "diagnostic",
            }
        batch_rows.append(
            _db_flush_row(agent, msg, ov_idx == msg_idx or msg is pending_cli_message)
        )
...
def _db_flush_write(
    agent, batch_rows: list[dict[str, Any]], batch_msgs: list[dict], messages: list[dict]
) -> None:
    ...
    agent._session_db.append_messages_batch(
        ...
        turn_lease_ttl_seconds=getattr(agent, "_active_session_turn_lease_ttl_seconds", 300.0)
        or 300.0,
    )
...
def _db_flush_adopt_compression_tip(agent) -> bool:
    ...
    if tip_row is None or tip_row.get("ended_at") is not None:
        return False
    logger.warning(
        "Adopted live compression tip %s for closed session %s; retrying flush once", tip, old_id
    )
...
def _db_flush_failed(
    agent, e: Exception, batch_rows: list[dict[str, Any]], adoption_budget: int
) -> bool:
    """Classify a failed flush; True when the caller should retry once on an adopted compression tip."""
    agent._db_flush_scan_prefix = (
        None  # full re-scan next flush: an exception mid-loop leaves mixed dispositions
    )
    ...
    from mertina.state import (
        StateDbCorruptError,
        StateDbReplacedError,
        classify_persistence_error,
        divert_session_transcript_jsonl,
    )
    from mertina.state_errors import CompressionSessionClosedError
    ...
    if isinstance(e, CompressionSessionClosedError):
        ...
        if adoption_budget > 0 and _db_flush_adopt_compression_tip(agent):
            return True
        agent._compression_adoption_failed = (
            True  # lets the turn explanation name rotation, not full-disk advice
        )
...
class SessionPersistenceMixin:
    ...
    def _drop_trailing_empty_response_scaffolding(self, messages: list[dict]) -> None:
        ...
        def tail(*keys: str) -> bool:
            return (
                ...
                and any(messages[-1].get(k) for k in keys)
            )
    ...
    _repair_message_sequence = _forward(
        "mertina.agent.agent_runtime_helpers", "repair_message_sequence"
    )
    ...
    _format_tools_for_system_message = _forward(
        "mertina.agent.system_prompt", "format_tools_for_system_message"
    )
    _convert_to_trajectory_format = _forward(
        "mertina.agent.agent_runtime_helpers", "convert_to_trajectory_format"
    )
    ...
    _extract_api_error_context = _forward_static(
        "mertina.agent.agent_runtime_helpers", "extract_api_error_context"
    )
    _dump_api_request_debug = _forward(
        "mertina.agent.agent_runtime_helpers", "dump_api_request_debug"
    )
