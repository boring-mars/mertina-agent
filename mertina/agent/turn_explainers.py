# Ported from hermes-agent agent/turn_explainers.py @ fbc4ea8b96
...
from mertina.agent.tool_result_classification import file_mutation_result_landed
...
def _file_mutation_identity(path: str, task_id: str | None) -> str:
    ...
    try:
        from mertina.tools.file_tools_paths import _resolve_path_for_task
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    try:
        ...
        if cached is not None:
            return cached
        try:
            from mertina.cli.config import load_config as _load_config
...
class TurnExplainersMixin:
        ...
        *,
        ...
        if landed:
            ...
            if mgr is not None and getattr(mgr, "enabled", False):
                ...
                if (
                    container_backend_for_task(task_id or "default") is None
                ):  # container paths carry no host ledger entry
            ...
            for path in targets:
                ...
                state.setdefault(
                    path,
                    {
                        ...
                        "identity": identity,
                        "stat": _file_stat_signature(identity),
                    },
                )
        else:
            cleared = {
                _file_mutation_identity(p, task_id) for p in (landed_paths if landed else targets)
            }
    ...
    @staticmethod
    def _file_mutations_still_failed(
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        return _display_flag_enabled(
            self,
            env_var="HERMES_FILE_MUTATION_VERIFIER",
            config_key="file_mutation_verifier",
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        return _display_flag_enabled(
            self,
            env_var="HERMES_TURN_COMPLETION_EXPLAINER",
            config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        persistence_cause: str | None = None,
        db_path=None,
        model: str = "",
    ) -> str:
        ...
        if body is None and reason == "session_persistence_failed":
            from mertina.constants import display_hermes_home, profile_cli_selector
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                from mertina.state import _default_db_path

                body = body.replace("{db_path}", str(db_path or _default_db_path()))
                body = body.replace("{backups_dir}", str(get_default_hermes_root() / "backups"))
