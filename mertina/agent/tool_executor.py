...
from mertina.tools.budget_config import DEFAULT_BUDGET, BudgetConfig, budget_for_context_window
...
from mertina.tools.tool_result_storage import (
    enforce_turn_budget,
    ...
    try:
        candidates = (
            ...
            if isinstance(function_result, str)
                ...
                *(
                    p.get("text")
                    for p in function_result.get("content") or []
...
def _ensure_file_checkpoint(
    ...
    if not file_path:
        return
    from mertina.agent.file_safety import is_nt_namespace_path
    from mertina.tools.file_tools_paths import _resolve_path_for_task, container_backend_for_task
    ...
    agent._checkpoint_mgr.ensure_checkpoint(
        agent._checkpoint_mgr.get_working_dir_for_path(str(resolved_path)),
...
def _authorization_gate_lock_timeout() -> float:
    ...
    """
    try:
        from mertina.tools.approval_human_wait import human_wait_ceiling
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    from mertina.agent.deadline import resolve_timeout
...
def _flush_session_db_after_tool_progress(agent, messages: list, *, stage: str) -> bool:
    ...
    from mertina.agent.conversation_loop import _maybe_inject_run_budget_wrapup
    from mertina.agent.turn_iteration_prep import _maybe_inject_iteration_budget_warning
    ...
    try:
        ...
        agent._incremental_persistence_failed = True
        from mertina.state import classify_persistence_error
...
def _image_generate_parallel_limit() -> int:
    ...
    try:
        from mertina.cli.config import load_config
...
def _is_interpreter_shutdown_submit_error(exc: RuntimeError) -> bool:
    ...
    #55924/#58720).
    """
    from mertina.tools.interpreter_shutdown import interpreter_shutting_down
    ...
    *,
    ...
    for tc in tool_calls:
        ...
        if hook_error_type is not None:
            _ToolCallRef(
                name, {}, effective_task_id, (hook_id or _pairing_tool_call_id)(tc), []
            ).emit_post(
                agent,
                result,
                status="cancelled",
...
def _tool_search_scoped_names(agent) -> frozenset:
    ...
    try:
        ...
        from mertina.tools import tool_search as _ts
        from mertina.tools.registry import registry as _registry
    except Exception:
        return frozenset()
...
def _canonical_tool_name(function_name: str) -> str:
    ...
    from mertina.model_tools import _LEGACY_TOOL_ALIASES as _lta
...
class _ConcurrentToolAuthorizationGate:
    ...
    def __init__(
        self, *, lock_timeout: float | None = None, session_key: str | None = None
    ) -> None:
        ...
        if self._session_key is None:
            ...
            try:
                from mertina.tools.approval_context import get_current_session_key
    ...
    def _human_wait_seconds(self) -> float:
        try:
            from mertina.tools.approval_human_wait import human_wait_seconds
...
def _set_worker_activity_callback(agent) -> None:
    ...
    with contextlib.suppress(Exception):
        from mertina.tools.environments.base import set_activity_callback
...
def _blocked_tool_result(
    ...
    *,
    ...
    if block_message is not None:
        ...
        error_message = (
            getattr(guardrail_decision, "message", None) or "Tool blocked by guardrail policy"
        )
    ref.emit_post(
        agent, result, status="blocked", error_type=error_type, error_message=error_message
    )
    return result
...
def _dispatch_authorized_once(
    ...
    *,
    ...
    if block_message is None:
        ...
        block_message, ref.args = (
            resolve() if authorization_gate is None else authorization_gate.run(resolve)
        )
        state.args = ref.args
    ...
    from mertina.agent.terminal_approval_batch import prepare_current_terminal
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    from mertina.cli.middleware import (
        apply_tool_request_middleware,
        run_tool_execution_middleware,
    )

    from mertina.agent import relay_tools
    ...
    from mertina.agent.terminal_approval_batch import bind_prepared_dispatch
    ...
    def _hermes_pipeline(relay_args: dict[str, Any]) -> Any:
        ...
        request_args = (
            request_result.payload if isinstance(request_result.payload, dict) else relay_args
        )
        trace.clear()
...
def _abandoned_sequential_result(
    agent, ref: _ToolCallRef, message: str, result_cls, **outcome
) -> _ManagedToolResult:
...
def _poll_sequential_future(
    agent, future, function_name: str, deadline: float | None, started: float, authorization_gate
) -> tuple[str, Any]:
...
def _run_sequential_tool_execution_middleware(
    ...
    *,
    ...
    from mertina.agent.terminal_approval_batch import take_prepared_call
    ...
    try:
        ...
        if state == "interrupted":
            ...
            interrupt_reason = (
                getattr(agent, "_tool_interrupt_reason", None) or "interrupt requested"
            )
            message = (
                f"[Tool execution cancelled — {function_name} was abandoned: {interrupt_reason}]"
            )
            logger.info(
...
def _begin_tool_execution(agent, ref: _ToolCallRef, display_index: int | None) -> None:
    ...
    if _tool_progress_enabled(agent):
        ...
        if agent.verbose_logging:
            ...
            print(
                f"  📞 {prefix}: {function_name}({list(function_args.keys())}) - {_preview(json.dumps(display_args, ensure_ascii=False), agent.log_prefix_chars)}"
            )
    ...
    with contextlib.suppress(Exception):
        if function_name in {"write_file", "patch"}:
            ...
            if _is_destructive_command(command):
                from mertina.tools.file_tools_paths import container_backend_for_task
...
def _commit_tool_result(
    ...
    *,
    ...
    """
    function_name, function_args, tool_call_id, effective_task_id = (
        ref.name,
        ref.args,
        ref.call_id,
        ref.task_id,
    ...
    if _is_multimodal_tool_result(persisted_result):
        persisted_result = _persist_multimodal_text_parts(
            ...
            function_name,
            tool_call_id,
            get_active_env(effective_task_id),
            budget,
        )
    else:
...
def _persist_multimodal_text_parts(
    result: dict, tool_name: str, tool_call_id: str, env, budget: BudgetConfig
) -> dict:
    ...
    for part in parts:
        ...
        if isinstance(text, str):
            replaced = maybe_persist_tool_result(
                content=text, tool_name=tool_name, tool_use_id=tool_call_id, env=env, config=budget
            )
            if replaced != text:
...
def _finalize_tool_batch(
    agent, messages: list, effective_task_id: str, num_tools: int, budget: BudgetConfig
) -> None:
...
def _print_tool_completed(agent, index: int, tool_duration: float, result) -> None:
    ...
    if agent.verbose_logging:
        ...
        print(
            f"  ✅ Tool {index} completed in {tool_duration:.2f}s - {_preview(result if isinstance(result, str) else str(result), agent.log_prefix_chars)}"
        )
...
class _ConcurrentBatch:
    ...
    def _dispatch_worker(
        self, index: int, ref: _ToolCallRef, scope_block, start_gate: _WorkerStartOnce
    ) -> _ToolOutcome | None:
        ...
        with _registered_tool_worker(agent) as _worker_tid:
            ...
            try:
                outcome = self._dispatch_worker(
                    index, pc.ref(self.effective_task_id), pc.scope_block, start_gate
                )
    ...
    def _running_names(self, not_done, future_to_index) -> list[str]:
        return [
            self.parsed_calls[future_to_index[f]].name for f in not_done if f in future_to_index
        ]
...
def _unfinished_tool_result(
    agent, ref: _ToolCallRef, *, timed_out: bool, timeout_s: float | None
    ...
    if timed_out:
        ...
        outcome = dict(
            status="error", error_type="thread_missing_result", error_message=function_result
        )
        tool_duration, effect_disposition = 0.0, None
...
def _append_batch_results(
    agent, messages: list, effective_task_id: str, batch: _ConcurrentBatch, budget: BudgetConfig
) -> bool:
    ...
    for i, pc in enumerate(batch.parsed_calls):
        ...
        if agent._should_emit_quiet_tool_messages():
            ...
            _print_tool_completed(
                agent, i + 1, tool_duration, _multimodal_text_summary(display_function_result)
            )
...
def execute_tool_calls_concurrent(
    ...
    *,
    ...
    spinner = _start_quiet_tool_spinner(
        agent, "", {}, label=f"⚡ running {num_tools} tools concurrently"
    )
    try:
        batch.run()
    finally:
        if spinner:
            ...
            spinner.stop(
                f"⚡ {len(finished)}/{num_tools} tools completed in {sum(r.duration for r in finished):.1f}s total"
            )
...
def _start_quiet_tool_spinner(
    agent, function_name: str, function_args: dict, *, gate: bool = True, label: str | None = None
):
    ...
    if not agent._should_emit_quiet_tool_messages() or (
        gate and not agent._should_start_quiet_spinner()
    ):
        return None
...
def _finish_quiet_tool_spinner(
    agent, spinner, function_name: str, function_args: dict, tool_duration: float, result
) -> None:
    ...
    if spinner or agent._should_emit_quiet_tool_messages():
        cute = _get_cute_tool_message_impl(
...
def _resolve_sequential_dispatch(agent, ref: _ToolCallRef, messages: list) -> _SequentialDispatch:
    ...
    if function_name != "delegate_task" and function_name in INLINE_TOOL_EXECUTORS:
        ...
        return _SequentialDispatch(
            lambda next_args: inline_executor(agent, next_args, inline_ctx), finish_in_finally=False
        )
    ...
    if agent._context_engine_tool_names and function_name in agent._context_engine_tool_names:
        return _SequentialDispatch(
            execute=lambda next_args: agent.context_compressor.handle_tool_call(
                function_name, next_args, messages=messages
            ),
            spinner=_start_quiet_tool_spinner(agent, function_name, function_args, gate=False),
            error_result=lambda e: json.dumps(
                {"error": f"Context engine tool '{function_name}' failed: {e}"}
            ),
    ...
    if agent._memory_manager and agent._memory_manager.has_tool(function_name):
        ...
        return _SequentialDispatch(
            execute=lambda next_args: agent._memory_manager.handle_tool_call(
                function_name, next_args
            ),
            spinner=_start_quiet_tool_spinner(agent, function_name, function_args),
            error_result=lambda e: json.dumps(
                {"error": f"Memory tool '{function_name}' failed: {e}"}
            ),
...
def _append_invalid_arguments_result(
    agent, messages: list, ref: _ToolCallRef, parse_error: str
) -> bool:
    ...
    try:
        ...
        if dispatch.finish_spinner and dispatch.finish_in_finally:
            _finish_quiet_tool_spinner(
                agent, dispatch.spinner, ref.name, ref.args, tool_duration, _spinner_result
            )
...
def execute_tool_calls_sequential(
    ...
    *,
    ...
    from mertina.agent.terminal_approval_batch import (
...
def execute_tool_calls_segmented(
    ...
    for kind, calls in segments:
        ...
        run_segment(
            agent, segment_message, messages, effective_task_id, api_call_count, finalize=False
        )
        if getattr(agent, "_incremental_persistence_failed", False):
            return
    ...
    if total_tools > 0:
        _finalize_tool_batch(
            agent, messages, effective_task_id, total_tools, _budget_for_agent(agent)
        )
