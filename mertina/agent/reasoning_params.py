...
def unset_reasoning_default(agent) -> dict | None:
    ...
    if not default:
        return None
    try:
        from mertina.agent.models_dev import get_model_capabilities
...
class ReasoningParamsMixin:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if base_url_host_matches(url, "nousresearch.com") or base_url_host_matches(
            url, "ai-gateway.vercel.sh"
        ):
            return True
        if base_url_host_matches(url, "models.github.ai") or base_url_host_matches(
            url, "githubcopilot.com"
        ):
            try:
                from mertina.cli.models import github_model_reasoning_efforts
        ...
        try:
            from mertina.cli.models_reasoning_caps import (
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        try:
            from mertina.cli.models_local import lmstudio_model_reasoning_options
        except Exception:
            return []
        return _cached_probe(
            self, "_lm_reasoning_opts_cache", lmstudio_model_reasoning_options, [], bool
        )

    def _ollama_supports_thinking_cached(self) -> bool:
        """True only if Ollama's ``/api/show`` declares the ``thinking`` capability."""
        try:
            from mertina.cli.models_local import ollama_model_supports_thinking
        except Exception:
            return False
        return bool(
            _cached_probe(
    ...
    _reapply_reasoning_echo_for_provider = _forward(
        "mertina.agent.agent_runtime_helpers", "reapply_reasoning_echo_for_provider"
    )
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict, model: "str | None" = None) -> dict:
        ...
        if not isinstance(tool_calls, list):
            return api_msg
        from mertina.agent.transports.chat_completions import _model_consumes_thought_signature

        strip = {"call_id", "response_item_id"} | (
            set() if _model_consumes_thought_signature(model) else {"extra_content"}
        )
        api_msg["tool_calls"] = [
    ...
    _sanitize_tool_call_arguments = _forward_static(
        "mertina.agent.agent_runtime_helpers", "sanitize_tool_call_arguments"
    )
