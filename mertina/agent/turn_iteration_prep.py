...
from mertina.agent.display import KawaiiSpinner
...
def _maybe_inject_iteration_budget_warning(agent: Any, messages: Any) -> bool:
    ...
    if (
        ...
        return False
    notice = ITERATION_BUDGET_WARNING_TEMPLATE.format(used=budget.used, maximum=budget.max_total)
    if kanban_worker:
        notice += (
...
def prepare_iteration(
    ...
    *,
    ...
    if getattr(agent, "_nous_wire_pending", None):
        from mertina.agent.nous_wire import apply_pending_wire_switch
    ...
    from mertina.agent.agent_runtime_helpers import repair_message_sequence_with_cursor
...
def _inject_steer_after_newest_tool_result(agent: Any, messages: Any, steer_text: str) -> None:
    ...
    for _si in range(len(messages) - 1, -1, -1):
        ...
        if isinstance(_sm, dict) and _sm.get("role") == "tool":
            from mertina.agent.prompt_builder import steer_user_row

            messages.insert(_si + 1, steer_user_row(steer_text))
            logger.debug(
                "Pre-API-call steer drain: appended user row after tool msg at index %d", _si
            )
            return
    from mertina.agent.agent_runtime_helpers import _requeue_pending_steer
...
def begin_iteration(
    ...
    *,
    ...
    if agent._interrupt_requested:
        ...
        _turn_exit_reason = (
            f"interrupted_by_system({_issuer})" if _issuer else "interrupted_by_user"
        )
        if not agent.quiet_mode:
