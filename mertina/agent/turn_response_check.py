...
from mertina.agent.turn_api_call import stop_thinking_spinner
...
from mertina.agent.turn_truncation import handle_content_policy_refusal, recover_from_truncation
...
def _derive_finish_reason(agent: Any, response: Any, messages: Any) -> str:
    if agent.api_mode == "codex_responses":
        ...
        if (
            ...
            and agent._get_transport().normalize_response(response).tool_calls
        ):
            return "length"
...
def check_api_response(
    ...
    *,
    ...
    from mertina.agent.turn_recovery import validate_response_shape
    ...
    if agent.provider == "nous":
        try:
            from mertina.agent.nous_rate_guard import clear_nous_rate_limit
...
def retry_invalid_response(
    ...
    *,
    ...
    from mertina.agent.conversation_loop import _arm_fallback_restart
    from mertina.agent.retry_utils import jittered_backoff
    ...
    if _soft is not None and (
        _soft.reason in (FailoverReason.rate_limit, FailoverReason.billing) or _soft.is_auth
    ):
        _recovered, _retry.has_retried_429 = agent._recover_with_credential_pool(
            status_code=None,
            has_retried_429=_retry.has_retried_429,
            classified_reason=_soft.reason,
            error_context=_soft_ctx,
            billing_unverified=_soft.billing_unverified,
        )
        if _recovered:
            agent._buffer_diagnostic_status(
                f"🔄 Codex soft failure ({_soft.reason.value}) — switched to the next pool credential, retrying..."
            )
    ...
    if retry_count >= max_retries:
        ...
        return _verdict(
            ...
            stamp_failure(
                ...
                invalid_response_failure_reason(response),
                True,
            ),
        )
