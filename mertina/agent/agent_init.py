...
from mertina.agent.agent_runtime_helpers import _ra
from mertina.agent.context_compressor import ContextCompressor
from mertina.agent.iteration_budget import IterationBudget, normalize_budget_warning_ratio
from mertina.agent.memory_manager import StreamingContextScrubber
...
from mertina.agent.process_bootstrap import _install_safe_stdio
from mertina.agent.session_activity import ActivityProvenance
from mertina.agent.subdirectory_hints import SubdirectoryHintTracker
from mertina.agent.think_scrubber import StreamingThinkScrubber
from mertina.agent.tool_guardrails import ToolCallGuardrailConfig, ToolCallGuardrailController
from mertina.cli.config import cfg_get
from mertina.cli.route_identity import normalize_route_base_url
from mertina.cli.timeouts import get_provider_request_timeout
from mertina.constants import get_hermes_home
from mertina.state_ids import new_session_id
from mertina.utils import base_url_host_matches, is_truthy_value
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from mertina.cli.providers import HERMES_OVERLAYS, get_provider
    ...
    with suppress(Exception):
        from mertina.cli.auth import PROVIDER_REGISTRY
        from mertina.cli.models import normalize_provider as normalize_model_provider
        from mertina.cli.providers import normalize_provider as normalize_registry_provider
...
def _context_route_mismatch(
    ...
    *,
    ...
    return bool(configured_provider and active_provider and configured_provider != active_provider)
...
def _codex_gpt55_autoraise_notice_seen(autoraise: dict[str, Any]) -> bool:
    ...
    try:
        ...
        return _codex_gpt55_autoraise_notice_marker().read_text(encoding="utf-8").strip() == current
...
def _custom_provider_model_matches(agent_model: str, entry: dict[str, Any]) -> bool:
    ...
    catalog = (
        [str(m).strip().lower() for m in models] if isinstance(models, (dict, list, tuple)) else []
    )
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    from mertina.agent.transports import registered_api_modes
    ...
    if is_actual_route(agent.provider, base_url):
        ...
        from mertina.cli.providers import nous_api_mode
        ...
        try:
            ...
            from mertina.cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    if credential_pool is not None:
        try:
            from mertina.agent.credential_pool import credential_pool_matches_provider
    ...
    if agent.provider == "nous":
        with suppress(Exception):
            from mertina.cli.nous_auth_keepalive import start_nous_auth_keepalive

            start_nous_auth_keepalive()

    with suppress(Exception):
        from mertina.cli.model_normalize import _AGGREGATOR_PROVIDERS, normalize_model_for_provider
    ...
    from mertina.cli.anon_auth import pin_model_for_route
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from mertina.agent.agent_runtime_helpers import cache_ttl_means_disabled
        from mertina.cli.config import load_config_readonly as _load_pc_cfg
...
def _init_turn_state(agent, run_budget_seconds):
    ...
    from mertina.agent.credits_tracker import new_credits_latch
...
def _setup_logging(agent):
    ...
    from mertina.logging import setup_logging, setup_verbose_logging
...
def _print_key_banner(key, label: str, warn_missing: bool = False) -> None:
    ...
    from mertina.agent.azure_identity_adapter import is_token_provider
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    from mertina.agent.anthropic_adapter import build_anthropic_client
    from mertina.agent.anthropic_credentials import resolve_anthropic_token
    ...
    if agent.provider == "bedrock":
        ...
        from mertina.agent.bedrock_adapter import bind_bedrock_runtime
    ...
    effective_key = (
        api_key
        or (
            resolve_anthropic_token(model=getattr(agent, "model", None))
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from mertina.cli.auth import build_minimax_oauth_token_provider
    ...
    from mertina.agent.anthropic_credentials import anthropic_route_is_oauth
...
def _init_moa_client(agent, api_key):
    ...
    from mertina.agent.moa_loop import bind_moa_runtime
...
def _explicit_client_kwargs(agent, api_key, base_url, _provider_timeout) -> dict[str, Any]:
    ...
    if _headers_for is not None:
        ...
        with suppress(Exception):
            from mertina.providers import get_provider_profile as _gpf
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> dict[str, Any] | None:
    ...
    """
    from mertina.agent.auxiliary_client import resolve_provider_client

    _routed_client, _ = resolve_provider_client(
        agent.provider or "auto", model=agent.model, raw_codex=True
    )
    if _routed_client is not None:
        from mertina.cli.providers import is_actual_route, normalize_provider
    ...
    for _fb in _fallback_entries(fallback_model):
        try:
            from mertina.cli.fallback_config import resolve_entry_api_key
    ...
    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
        ...
        from mertina.agent.auxiliary_unavailable import missing_provider_credentials_message
...
def _apply_openai_header_policy(agent, client_kwargs: dict[str, Any]) -> None:
    ...
    from mertina.agent.anthropic_adapter import _TOOL_STREAMING_BETA
    ...
    try:
        from mertina.cli.config import (
...
def _init_openai_client(agent, api_key, base_url, fallback_model, _provider_timeout):
    ...
    try:
        from mertina.agent.ssl_guard import verify_ca_bundle
        ...
        if not agent.quiet_mode:
            ...
            _print_key_banner(
                client_kwargs.get("api_key", "none"),
                "API key",
...
_HOST_DEFAULT_HEADERS: list[tuple[str, Callable[[Any, str], dict[str, str]]]] = [
    ("openrouter.ai", _lazy_headers("mertina.agent.auxiliary_client", "build_or_headers")),
    (
        "integrate.api.nvidia.com",
        _lazy_headers("mertina.agent.auxiliary_client", "build_nvidia_nim_headers", pass_base=True),
    ),
    ("api.routermint.com", _lazy_headers("mertina.agent.client_lifecycle", "_routermint_headers")),
    ("githubcopilot.com", _lazy_headers("mertina.cli.models", "copilot_default_headers")),
    ("api.kimi.com", lambda _k, _b: {"User-Agent": "claude-code/0.1.0"}),
    ("portal.qwen.ai", _lazy_headers("mertina.agent.client_lifecycle", "_qwen_portal_headers")),
    (
        "chatgpt.com",
        _lazy_headers("mertina.agent.codex_headers", "codex_cloudflare_headers", pass_key=True),
    ),
    ("x.ai", _lazy_headers("mertina.tools.xai_http", "hermes_xai_default_headers")),
]
...
def _init_fallback_chain(agent, fallback_model):
    ...
    from mertina.agent.agent_runtime_helpers import sync_credential_pool_entry_id
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    ...
    try:
        from mertina.cli.plugins import discover_plugins
    ...
    try:
        from mertina.tools.registry import registry as _snapshot_registry
    ...
    from mertina.agent.oneshot_footprint import prune_oneshot_tools
    ...
    # owning a task.
    from mertina.agent.delegation_context import owned_kanban_task
    from mertina.agent.prompt_builder import KANBAN_GUIDANCE
...
def _publish_session_id(session_id: str) -> None:
    ...
    """
    try:
        from mertina.gateway.session_context import set_current_session_id
...
def _init_session_state(
    ...
    from mertina.tools.checkpoint_manager import CheckpointManager
    ...
    with suppress(Exception):
        from mertina.tools.approval import _YOLO_MODE_FROZEN

        if _YOLO_MODE_FROZEN:
            agent._session_init_model_config["yolo_mode"] = True

    # In-memory todo list for task planning (one per agent/session)
    from mertina.tools.todo_tool import TodoStore
...
def _apply_display_config(agent, _agent_cfg, platform):
    ...
    _load_mode = (
        str(_model_section.get("lmstudio_load_mode", "explicit") or "explicit").strip().lower()
    )
...
def _init_memory(agent, _agent_cfg, skip_memory, platform):
    ...
    # normal path here.)
    _memory_toolset_requested = "memory" in (agent.enabled_toolsets or []) and "memory" not in (
        agent.disabled_toolsets or []
    )
    if not skip_memory or _memory_toolset_requested:
        # Memory is optional — don't break agent init
        with suppress(Exception):
            from mertina.tools.memory_tool import (
                MemoryStore,
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                from mertina.agent.memory_manager import MemoryManager as _MemoryManager
                from mertina.plugins.memory import load_memory_provider as _load_mem

                agent._memory_manager = _MemoryManager()
                _mp = _load_mem(_mem_provider_name)
                if _mp is None:
                    # The provider left core for the catalog (or was never installed): fetch it once.
                    from mertina.cli.memory_provider_migration import recover_at_startup
                ...
                if agent._memory_manager.providers:
                    ...
                    _ra().logger.debug(
                        "Memory provider '%s' not found or not available", _mem_provider_name
                    )
                    agent._memory_manager = None
    ...
    from mertina.agent.memory_manager import inject_memory_provider_tools
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    from mertina.agent.empty_response_guard import resolve_guard_settings

    (agent._empty_guard_enabled, agent._empty_guard_cost_threshold_usd) = resolve_guard_settings(
    ...
    if _verbosity and _verbosity not in {"low", "medium", "high"}:
        logger.warning(
            "Unknown agent.text_verbosity %r; expected low, medium or high — ignoring", _verbosity
        )
        _verbosity = ""
    ...
    if agent._environment_probe:
        with suppress(Exception):
            from mertina.tools.env_probe import warm_environment_probe_async
...
def _compression_threshold(agent, cfg: dict[str, Any]) -> tuple[float, bool]:
    ...
    with suppress(Exception):
        from mertina.agent.auxiliary_client import (
...
def _custom_provider_configured_base_url(
    ...
    if isinstance(_user_providers, dict):
        from mertina.cli.config import is_provider_enabled
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from mertina.cli.auth import resolve_provider as resolve_auth_provider
...
def _scope_context_length_to_default_runtime(
    ...
    if isinstance(_default, dict):
        from mertina.cli.config import split_model_config_default
    ...
    if _configured_default_model:
        with suppress(Exception):
            from mertina.cli.model_normalize import normalize_model_for_provider
...
def config_context_length_for_runtime(agent, config=None) -> int | None:
    ...
    """
    try:
        from mertina.cli.config import get_compatible_custom_providers, load_config
        ...
        return _scope_context_length_to_default_runtime(
            ...
            _agent_cfg,
            _model_section,
            get_compatible_custom_providers(_agent_cfg),
            _pin,
            str(getattr(agent, "base_url", "") or ""),
        )
    except Exception:
        logger.debug(
            "Could not re-read model.context_length for the current runtime", exc_info=True
        )
        return None
...
def _resolve_context_length(agent, _agent_cfg, base_url):
    ...
    try:
        from mertina.cli.config import get_compatible_custom_providers
    ...
    if _config_context_length is None and _custom_providers:
        with suppress(Exception):
            from mertina.cli.config import get_custom_provider_context_length
    ...
    if _config_context_length is not None:
        from mertina.agent.context_pin import warn_once_on_pin_disagreement
...
def _select_context_engine(_agent_cfg):
    ...
    _copy_failed = False
    try:
        from mertina.plugins.context_engine import load_context_engine
    ...
    if _selected_engine is None:
        try:
            from mertina.cli.plugins import get_plugin_context_engine
...
def _compressor_max_tokens(agent):
    ...
    with suppress(Exception):
        from mertina.agent.gemini_native_adapter import (
...
def _build_context_engine(
    ...
    if _selected_engine is not None:
        ...
        from mertina.agent.model_metadata import get_model_context_length
    ...
    from mertina.agent.conversation_compression import (
    ...
    from mertina.agent.native_compaction import resolve_native_compaction_capabilities
...
def _warn_nonagentic_hermes_model(agent):
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from mertina.cli.model_switch import _check_hermes_model_warning
...
def _inject_context_engine_tools(agent):
    ...
    if (
        ...
        from mertina.agent.memory_manager import normalize_tool_schema
...
def _emit_compression_summary(agent, cs):
    ...
    if not agent.quiet_mode:
        ...
        if cs.enabled:
            ...
            _cap_binds = (
                bool(_cap) and _cap > 0 and _cc.threshold_tokens == min(_cap, _cc.context_length)
            )
...
def _init_usage_state(agent):
    from mertina.agent.runtime_cwd import scope_terminal_cwd
...
def init_agent(
    ...
    if is_actual_route(provider, base_url):
        from mertina.cli.auth import normalize_actual_base_url
    ...
    try:
        from mertina.cli.config import load_config_readonly as _load_agent_config
...
_PLUGIN_COMPAT_LAZY = {
    "ToolGuardrailDecision": ("mertina.agent.tool_guardrails", "ToolGuardrailDecision"),
}
