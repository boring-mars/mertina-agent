...
from mertina.agent.message_sanitization import _sanitize_messages_surrogates
from mertina.agent.prompt_caching import build_prompt_cache_plan, effective_cache_ttl
from mertina.agent.turn_context import build_api_messages
from mertina.agent.usage_anchor import anchored_context_tokens
...
def _append_moa_context(
    agent: Any, api_messages: Any, moa_config: Any, original_user_message: Any
) -> None:
    ...
    try:
        from mertina.agent.message_content import flatten_message_text as _flatten_mt
        from mertina.agent.moa_loop import _preset_temperature, aggregate_moa_context
        ...
        _moa_context = aggregate_moa_context(
            ...
            degraded_reference_policy=str(moa_config.get("degraded_reference_policy") or "loud"),
            agent=agent,
        )
        if not _moa_context:
            return
...
def assemble_api_request(
    ...
    *,
    ...
    from mertina.agent.model_metadata import estimate_messages_tokens_rough
    ...
    try:
        from mertina.agent.agent_runtime_helpers import consume_pending_sanitizer_heal_notice
    ...
    from mertina.agent.turn_context import _agent_stale_thinking_on_wire
