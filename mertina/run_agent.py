...
from mertina.constants import get_hermes_home
...
def _launch_cwd_for_session(source: str) -> str | None:
    ...
    """
    if source not in CLI_FAMILY_SOURCES or (
        os.environ.get("TERMINAL_ENV") or "local"
    ).strip().lower() not in ("", "local"):
        return None
    try:
        return os.getcwd()
...
def _session_source_for_agent(platform: str | None) -> str:
    try:
        from mertina.gateway.session_context import get_session_env
    except Exception:
...
def _gateway_origin_json(agent: "AIAgent") -> str | None:
    ...
    if not profile:
        try:
            from mertina.cli.profiles import get_active_profile_name
...
from mertina.cli.env_loader import load_hermes_dotenv
from mertina.cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
...
from mertina.agent.activity_tracking import ActivityTrackingMixin
from mertina.agent.api_error_summary import PROVIDER_STREAM_PARSE_MARKERS, ApiErrorSummaryMixin
from mertina.agent.api_request_hooks import ApiRequestHooksMixin
...
from mertina.agent.compression_facade import CompressionFacadeMixin
...
from mertina.agent.memory_provider import is_trivial_prompt
...
from mertina.agent.rate_limit_credits import RateLimitCreditsMixin
from mertina.agent.reasoning_params import ReasoningParamsMixin
from mertina.agent.session_activity import ActivityProvenance
from mertina.agent.session_persistence import SessionPersistenceMixin
from mertina.agent.status_output import StatusOutputMixin
from mertina.agent.stream_delivery import StreamDeliveryMixin
from mertina.agent.tool_guardrails import (
    ToolGuardrailDecision,
    append_toolguard_guidance,
    toolguard_synthetic_result,
)
from mertina.agent.turn_explainers import TurnExplainersMixin
...
from mertina.model_tools import get_toolset_for_tool
from mertina.tools.interrupt import set_interrupt as _set_interrupt
from mertina.tools.terminal_tool_lifecycle import get_active_env
...
def _review_should_defer(agent: Any, task_cfg: dict[str, Any] | None) -> bool:
    ...
    from mertina.agent.review_idle_queue import defer_mode, review_targets_managed_local
    ...
    def __init__(
        ...
        *,
        ...
        self.body: dict[str, Any] = {
            "error": {"message": message, "code": code, "param": param, "type": "error"}
        }
...
class AIAgent(
    ...
    def __init__(
        ...
        connection_callback: callable = None,
    ...
    def _get_session_db_for_recall(self):
        ...
        try:
            from mertina.state_registry import acquire
    ...
    def _session_row_model_config(self) -> Any:
        ...
        try:
            from mertina.tools.approval import is_session_yolo_enabled
    ...
    def _ensure_db_session(self) -> None:
        ...
        if (
            ...
            or self._session_db_created
            or not self._session_db
        ):
            return
    ...
    def _transition_context_engine_session(
        ...
        **extra_context,
        ...
        should_start = bool(
            old_session_id or previous_messages is not None or carry_over_context or extra_context
        )
    ...
    def _effective_lmstudio_context_length(
        config_context_length: int | None, runtime_context_length: Any
    ) -> int | None:
        """Return a safe context budget from explicit intent and verified runtime."""
        explicit = _positive_int(config_context_length)
        runtime = _positive_int(
            getattr(runtime_context_length, "context_length", runtime_context_length)
        )
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: int | None = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from mertina.cli.models_local import ensure_lmstudio_model_loaded
    ...
    switch_model = _forward("mertina.agent.agent_runtime_helpers", "switch_model")
    ...
    _stream_diag_init = _forward_static("mertina.agent.stream_diag", "stream_diag_init")
    _stream_diag_capture_response = _forward(
        "mertina.agent.stream_diag", "stream_diag_capture_response"
    )
    _flatten_exception_chain = _forward_static(
        "mertina.agent.stream_diag", "flatten_exception_chain"
    )
    ...
    def _is_provider_stream_parse_error(self, error: BaseException) -> bool:
        ...
        return (
            ...
            and any(
                marker in str(error).strip().lower() for marker in PROVIDER_STREAM_PARSE_MARKERS
            )
        )

    _log_stream_retry = _forward("mertina.agent.stream_diag", "log_stream_retry")
    _emit_stream_drop = _forward("mertina.agent.stream_diag", "emit_stream_drop")
    ...
    _check_compression_model_feasibility = _forward(
        "mertina.agent.conversation_compression", "check_compression_model_feasibility"
    )
    _replay_compression_warning = _forward(
        "mertina.agent.conversation_compression", "replay_compression_warning"
    )
    ...
    def _hostname_for(self, base_url: str | None) -> str:
        ...
        return getattr(self, "_base_url_hostname", "") or base_url_hostname(
    ...
    def _is_azure_openai_url(self, base_url: str = None) -> bool:
        ...
        url = (
            str(base_url).lower()
            if base_url is not None
            else (getattr(self, "_base_url_lower", "") or "")
        )
    ...
    def _is_github_copilot_url(self, base_url: str = None) -> bool:
        ...
        return bool(hostname) and (
            hostname == "api.githubcopilot.com" or hostname.endswith(".githubcopilot.com")
        )
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        from mertina.agent.reasoning_timeouts import get_reasoning_stale_timeout_floor
    ...
    def _compute_non_stream_stale_timeout(self, api_payload: Any) -> float:
        ...
        from mertina.agent.chat_completion_helpers import (
        ...
        timeout = (
            max(stale_base, 240.0)
            if est_tokens > 100_000
            else max(stale_base, 150.0)
    ...
    def _is_copilot_url(self) -> bool:
        ...
        return any(
            base_url_host_matches(self._base_url_lower, h)
            for h in ("api.githubcopilot.com", "models.github.ai")
        )
    ...
    def _is_copilot_provider(self) -> bool:
        ...
        return (self.provider or "").strip().lower() in {
            "copilot",
            "github-copilot",
            "github",
        } or self._is_copilot_url()
    ...
    _anthropic_prompt_cache_policy = _forward(
        "mertina.agent.agent_runtime_helpers", "anthropic_prompt_cache_policy"
    )
    _direct_native_anthropic_tool_cache_capability = _forward(
        "mertina.agent.agent_runtime_helpers", "_direct_native_anthropic_tool_cache_capability"
    )
    ...
    def _model_requires_responses_api(model: str) -> bool:
        ...
        return (
            model.lower().rsplit("/", 1)[-1].startswith("gpt-5")
        )  # strip vendor prefix ("openai/gpt-5.4")

    @staticmethod
    def _provider_model_requires_responses_api(model: str, *, provider: str | None = None) -> bool:
        """Return True when this provider/model pair should use Responses API."""
        from mertina.cli.providers import is_actual_route
        ...
        from mertina.cli.runtime_provider_backends import _is_external_process_provider

        if _is_external_process_provider(normalized_provider):
            return False
        if normalized_provider == "copilot":
            try:
                from mertina.cli.models import _should_use_copilot_responses_api
    ...
    _looks_like_codex_intermediate_ack = _forward(
        "mertina.agent.agent_runtime_helpers", "looks_like_codex_intermediate_ack"
    )
    _extract_reasoning = _forward("mertina.agent.agent_runtime_helpers", "extract_reasoning")
    _cleanup_task_resources = _forward(
        "mertina.agent.chat_completion_helpers", "cleanup_task_resources"
    )

    # Background memory/skill review — prompts live in agent.background_review.
    from mertina.agent.background_review import (
        _COMBINED_REVIEW_PROMPT,
        _MEMORY_REVIEW_PROMPT,
        _SKILL_REVIEW_PROMPT,
    )

    _summarize_background_review_actions = _forward_static(
        "mertina.agent.background_review", "summarize_background_review_actions"
    )
    ...
    def _spawn_background_review(
        ...
        task_cfg = None
        if focus is None:
            from mertina.agent.background_review import load_background_review_settings
        ...
        from mertina.agent.turn_finalizer import _clone_background_review_messages
        ...
        if focus is None and not explicit and _review_should_defer(self, task_cfg):
            from mertina.agent.review_idle_queue import QUEUE
    ...
    def _spawn_background_review_now(
        ...
        from mertina.tools.thread_context import propagate_context_to_thread
    ...
    _build_memory_write_metadata = _forward(
        "mertina.agent.background_review", "build_memory_write_metadata"
    )
    _apply_pending_steer_to_tool_results = _forward(
        "mertina.agent.agent_runtime_helpers", "apply_pending_steer_to_tool_results"
    )

    def get_activity_summary(self) -> dict:
        """Diagnostic snapshot: ``last_activity_*`` plus the short aliases gateway and delegate readers use."""
        from mertina.agent.session_activity import build_activity_snapshot
        ...
        return build_activity_snapshot(
            ...
            else ActivityProvenance.UNKNOWN,
            extra={
    ...
    def release_clients(self) -> None:
        ...
        _quietly(
            self._drop_shared_client,
            lambda c: self._retire_shared_openai_client(c, reason="cache_evict"),
        )
    ...
    def _trim_process_memory() -> None:
        ...
        from mertina.cli.mem_trim import trim_memory
    ...
    def _finalize_owned_session_row(self) -> None:
        ...
        if getattr(self, "_owns_session_db", False) and session_db is not None:
            ...
            # See #90837.
            from mertina.state_registry import release_or_close
    ...
    def _latest_todo_response(self, history: list[dict[str, Any]]) -> tuple | None:
        ...
        from mertina.tools.todo_tool import MAX_TODO_RESULT_CHARS
        ...
        for idx in range(len(history) - 1, -1, -1):
            ...
            if (
                ...
                or not self._tool_response_matches_todo_call(history, idx)
            ):
                continue
            if len(content) > MAX_TODO_RESULT_CHARS:
                logger.warning(
                    "Skipping oversized todo tool response during hydration: session=%s chars=%d",
    ...
    def _tool_response_matches_todo_call(
        cls, history: list[dict[str, Any]], tool_index: int
    ) -> bool:
        ...
        tool_call_id = (
            history[tool_index].get("tool_call_id") if 0 <= tool_index < len(history) else None
        )
        if not tool_call_id:
            return False
    ...
    def _assistant_has_todo_tool_call(
        cls, assistant_msg: dict[str, Any], tool_call_id: str
    ) -> bool:
        ...
        return isinstance(tool_calls, list) and any(
            ...
            and cls._get_tool_call_name_static(tc) == "todo"
    ...
    _sanitize_api_messages = _forward_static(
        "mertina.agent.agent_runtime_helpers", "sanitize_api_messages"
    )

    @staticmethod
    def _is_thinking_only_assistant(
        msg: dict[str, Any], *, drop_codex_reasoning_items: bool = True
    ) -> bool:
        ...
        # #82108.
        from mertina.agent.native_compaction import has_compaction_checkpoint
        ...
        if drop_codex_reasoning_items and isinstance(codex_items, list):
            return any(
                isinstance(item, dict) and item.get("type") == "reasoning" for item in codex_items
            )
        return False
    ...
    _drop_thinking_only_and_merge_users = _forward_static(
        "mertina.agent.agent_runtime_helpers", "drop_thinking_only_and_merge_users"
    )
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        from mertina.tools.delegate_tool import _get_max_concurrent_children
    ...
    _repair_tool_call = _forward("mertina.agent.agent_runtime_helpers", "repair_tool_call")
    _invalidate_system_prompt = _forward("mertina.agent.system_prompt", "invalidate_system_prompt")
    ...
    _interruptible_streaming_api_call = _forward(
        "mertina.agent.chat_completion_helpers", "interruptible_streaming_api_call"
    )
    _try_activate_fallback = _forward(
        "mertina.agent.chat_completion_helpers", "try_activate_fallback"
    )
    ...
    def _has_pending_fallback(self) -> bool:
        ...
        See #17446.
        """
        return getattr(self, "_fallback_index", 0) < len(
            getattr(self, "_fallback_chain", None) or []
        )

    _restore_primary_runtime = _forward(
        "mertina.agent.agent_runtime_helpers", "restore_primary_runtime"
    )
    _try_recover_primary_transport = _forward(
        "mertina.agent.agent_runtime_helpers", "try_recover_primary_transport"
    )
    ...
    def _append_guardrail_observation(
        ...
        *,
        ...
        if decision.should_halt:
            ...
            if streak_halt is not None and streak_halt.code in (
    ...
    def _execute_tool_calls(
        ...
        self._executing_tools = (
            True  # allow _vprint during tool execution even with stream consumers
        )
        try:
            if len(tool_calls) <= 1:
                self._execute_tool_calls_sequential(*args)
            else:
                from mertina.agent.tool_dispatch_helpers import _plan_tool_batch_segments
                ...
                if len(segments) == 1:
                    run = (
                        ...
                        else self._execute_tool_calls_sequential
                    )
                    run(*args)
                else:
                    from mertina.agent.tool_executor import execute_tool_calls_segmented
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        from mertina.tools.delegate_tool import _strip_model_hidden_task_fields
        from mertina.tools.delegate_tool import delegate_task as _delegate_task
...
def _print_tool_listing() -> None:
    ...
    from mertina.model_tools import get_all_tool_names, get_available_toolsets
    from mertina.toolsets import get_all_toolsets, get_toolset_info
    ...
    for name in get_all_toolsets():
        ...
        if info:
            bucket = (
                ...
                else composite_toolsets
                if name in _COMPOSITE_TOOLSETS
                else scenario_toolsets
            )
    ...
    for name, info in basic_toolsets:
        ...
        print(
            f"    Tools: {', '.join(info['resolved_tools']) if info['resolved_tools'] else 'none'}"
        )
...
def _save_sample_trajectory(agent: "AIAgent", result: dict, user_query: str, model: str) -> None:
    ...
    entry = {
        "conversations": agent._convert_to_trajectory_format(
...
_PLUGIN_COMPAT_LAZY = {
    "COMPRESSED_SUMMARY_METADATA_KEY": (
        "mertina.agent.context_compressor",
        "COMPRESSED_SUMMARY_METADATA_KEY",
    ),
    "ContextCompressor": ("mertina.agent.context_compressor", "ContextCompressor"),
    "DEFAULT_AGENT_IDENTITY": ("mertina.agent.prompt_builder", "DEFAULT_AGENT_IDENTITY"),
    "FailoverReason": ("mertina.agent.error_classifier", "FailoverReason"),
    "OpenAI": ("mertina.agent.process_bootstrap", "OpenAI"),
    "atomic_json_write": ("mertina.utils", "atomic_json_write"),
    "build_context_files_prompt": ("mertina.agent.prompt_builder", "build_context_files_prompt"),
    "build_environment_hints": ("mertina.agent.prompt_builder", "build_environment_hints"),
    "build_skills_system_prompt": ("mertina.agent.prompt_builder", "build_skills_system_prompt"),
    "check_toolset_requirements": ("mertina.model_tools", "check_toolset_requirements"),
    "convert_scratchpad_to_think": ("mertina.agent.trajectory", "convert_scratchpad_to_think"),
    "estimate_request_tokens_rough": (
        "mertina.agent.model_metadata",
        "estimate_request_tokens_rough",
    ),
    "file_mutation_result_landed": (
        "mertina.agent.tool_result_classification",
        "file_mutation_result_landed",
    ),
    "flatten_message_text": ("mertina.agent.message_content", "flatten_message_text"),
    "get_tool_definitions": ("mertina.model_tools", "get_tool_definitions"),
    "handle_function_call": ("mertina.model_tools", "handle_function_call"),
    "is_truthy_value": ("mertina.utils", "is_truthy_value"),
    "jittered_backoff": ("mertina.agent.retry_utils", "jittered_backoff"),
    "load_soul_md": ("mertina.agent.prompt_builder", "load_soul_md"),
    "normalize_usage": ("mertina.agent.usage_pricing", "normalize_usage"),
    "redact_sensitive_text": ("mertina.agent.redact", "redact_sensitive_text"),
    "request_hard_interrupt": ("mertina.agent.interrupt_compat", "request_hard_interrupt"),
    "sanitize_context": ("mertina.agent.memory_manager", "sanitize_context"),
    "user_originated_turn_view": ("mertina.agent.context_compressor", "user_originated_turn_view"),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    from mertina.cli.plugin_compat import warn_once
