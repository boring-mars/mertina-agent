...
from mertina.tools.arg_coercion import coerce_tool_args
from mertina.toolsets import resolve_toolset, validate_toolset
from mertina.utils import file_signature
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    try:
        from mertina.agent.delegation_context import is_dispatcher_owned_worker_context
...
def _tool_defs_cache_key(
    ...
    if profile_scope == CHECK_FN_CACHE_BYPASS:
        return None
    try:
        from mertina.cli.config import get_config_path
...
def _apply_toolset_selection(
    tools: set, names: list[str], quiet_mode: bool, *, disable: bool
) -> None:
    """Add (or subtract) every toolset in *names* to/from *tools*, printing the selection unless quiet."""
    from mertina.toolsets import bundle_non_core_tools, get_toolset
    ...
    for name in names:
        if validate_toolset(name):
            ...
            if disable and (name.startswith("hermes-") or (get_toolset(name) or {}).get("posture")):
                ...
                if (
                    not quiet_mode
                    and name.startswith("hermes-")
                    and name not in _WARNED_DISABLED_BUNDLES
                ):
...
def _select_tool_names(
    enabled_toolsets: list[str] | None, disabled_toolsets: list[str] | None, quiet_mode: bool
) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        from mertina.toolsets import get_all_toolsets
...
def _rewrite_execute_code(td: dict[str, Any], available: set) -> dict[str, Any] | None:
    ...
    from mertina.tools.code_execution_tool import (
    ...
    return _fn_def(
        build_execute_code_schema(SANDBOX_ALLOWED_TOOLS & available, mode=_get_execution_mode())
    )
...
def _discord_rewriter(schema_fn_name: str):
    ...
    def _rewrite(td: dict[str, Any], available: set) -> dict[str, Any] | None:
        try:
            from mertina.tools import discord_tool as _dt
...
def _rewrite_browser_vault(td: dict[str, Any], available: set) -> dict[str, Any] | None:
    ...
    fn = td["function"]
    return _fn_def(
        {**fn, "description": fn.get("description", "").replace(_VAULT_INPUT_TOOL_HINT, concrete)}
    )


_VAULT_NO_PASSWORD_NOTE = (
    " Vault note: on a login/checkout form call browser_vault_list first, then browser_vault_fill, or "
...
def _compute_tool_definitions(
    ...
    from mertina.tools.kanban_toolset_context import scoped_kanban_toolset_selection

    with scoped_kanban_toolset_selection(enabled_toolsets):
        filtered_tools = _apply_dynamic_schemas(
    ...
    try:
        from mertina.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    try:
        from mertina.tools.tool_search import assemble_tool_defs
        from mertina.tools.tool_search import load_config as _load_ts_config

        ts_cfg = _load_ts_config()
        if not skip_tool_search_assembly and ts_cfg.enabled != "off":
            assembly = assemble_tool_defs(
                filtered_tools, context_length=_resolve_active_context_length(), config=ts_cfg
            )
...
def _resolve_active_context_length() -> int:
    ...
    try:
        ...
        if not model_id:
            return 0
        from mertina.agent.model_metadata import get_cached_context_length, get_model_context_length
        ...
        if provider:
            ...
            try:
                from mertina.cli.runtime_provider import resolve_runtime_provider
...
def _tool_result_observer_fields(tool_name: str, result: Any) -> tuple[str, str | None, str | None]:
    ...
    try:
        ...
        pass
    try:
        from mertina.agent.display import _detect_tool_failure
...
def _emit_post_tool_call_hook(
    *,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from mertina.cli.lifecycle import has_hook, invoke_hook
...
def _dispatch_bridge_tool(
    ...
    """
    try:
        from mertina.tools import tool_search as ts
    except Exception:
        return None
...
def _pre_dispatch_guards(
    ...
    try:
        from mertina.acp_adapter.edit_approval import maybe_require_edit_approval
        ...
        if function_name in {"write_file", "patch"}:
            return function_args, (
                tool_error("Edit approval denied: approval guard failed"),
                "edit_approval_error",
                None,
            )
...
def _approval_observability(ids: _CallIds):
    ...
    try:
        from mertina.tools.approval_context import (
...
def _execute_tool(
    ...
    *,
    ...
    def _dispatch(next_args: dict[str, Any]) -> Any:
        from mertina.tools.connectors import dispatch_connector_call, is_connector_name
...
def handle_function_call(
    ...
    if bridged is not None:
        ...
        from mertina.tools.connectors import CONNECTOR_BATCH_SENTINEL, dispatch_connector_batch
    ...
    from mertina.tools.connectors import is_connector_name
    from mertina.tools.connectors.gateway.names import parse_connector_name

    if function_name == "manage_connections" or is_connector_name(function_name):
        if "manage_connections" not in _select_tool_names(
        ...
        if is_connector_name(function_name) and parse_connector_name(function_name) is None:
            return _emit(
                tool_error(
                    "Malformed connector tool name; expected connectors__<connector>__<tool>."
                )
            )

    original_args = dict(function_args)
    if not skip_tool_request_middleware:
        function_args, original_args, trace = _apply_request_middleware(
            function_name, function_args, ids, trace
        )
    ...
    try:
        ...
        if blocked is not None:
            ...
            return _emit(
                result, status="blocked", error_type=error_type, error_message=error_message
            )

        # Any non-read/search tool resets the consecutive-read-loop counter.
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from mertina.tools.file_tools_read_tracking import notify_other_tool_call
        ...
        return _apply_transform_tool_result_hook(
            function_name, function_args, result, duration_ms, ids
        )
