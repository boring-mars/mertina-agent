...
# 已注释的主要功能：
#
# - Hermes 环境启动、网关来源、会话身份及旧模块辅助函数；
# - 原 13 个 Mixin、宽构造器、会话数据库、记忆、评审、客户端生命周期和 todo；
# - 多供应商适配、备用模型、压缩、子 Agent、工具防停滞和旧工具执行入口；
# - 原工具集 CLI、轨迹保存和插件兼容导出。
#
# 源代码改动点：
...
"""v0.1 的 Agent 入口：持有模型配置，转发对话循环和一轮工具调用。"""
...
class AIAgent:
    """封装最小模型配置与停止状态；对话和工具执行仍由各自模块负责。"""

    # [改动][溯源] 原 __init__ 位于基线 run_agent.py:251-298；
    # ROADMAP.md:70-85 只要求模型端点、密钥、模型名和每轮迭代上限。
    def __init__(
        self,
        base_url: str,
        model: str,
        ...
        *,
        ...
        """保存 v0.1 配置；可注入测试客户端，首次对话时才创建默认客户端。"""
        if not base_url or not model:
            raise ValueError('base_url and model are required')
        if isinstance(max_iterations, bool) or not isinstance(max_iterations, int) or max_iterations < 1:
            raise ValueError('max_iterations must be a positive integer')
        # [改动][溯源] ROADMAP.md:75-76,82：重试次数和搜索 provider 由当前最小循环使用。
        ...
        self._parallel_tool_names = frozenset({'web_search'})
        # [改动][溯源] ROADMAP.md:82 只开放 web_search；
        # agent/web_search_provider.py:BraveSearchProvider.is_available/search 表明 Brave 请求需要密钥，
        # agent/tool_executor.py:1837-1844 通过 valid_tool_names 拒绝其他工具。
        # self.valid_tool_names = frozenset({'web_search'})
        # [改动][溯源] agent/web_search_provider.py:get_provider_env 会去掉空白；
        # 这里采用相同判断，避免只设置空白环境变量时向模型误报搜索可用。
        # has_search = search_provider is not None or bool(os.getenv('BRAVE_SEARCH_API_KEY'))
        has_search = search_provider is not None or bool((os.getenv('BRAVE_SEARCH_API_KEY') or '').strip())
    ...
    def run_conversation(
        ...
        """将一轮对话交给现有循环，并返回消息、元数据和最终回答。"""
        from agent.conversation_loop import run_conversation

        if self.client is None:
            from openai import OpenAI

            self.client = OpenAI(base_url=self.base_url, api_key=self.api_key or os.getenv('OPENAI_API_KEY'))
        ...
        return run_conversation(
            ...
            task_id=task_id,
            stream_callback=stream_callback or self.stream_delta_callback,
        )

    # [改动][溯源] ROADMAP.md:68-69 要求 chat 只返回最终文本；
    # 原实现通过基线 run_agent.py:137 的 TurnFacadeMixin 提供入口。
    def chat(
        self,
        user_message: Any,
        system_message: str | None = None,
        ...
        """返回本轮最终文本；完整消息和用量由 run_conversation 提供。"""
        result = self.run_conversation(user_message, system_message, conversation_history)
        # [改动][溯源] ROADMAP.md:68-69：CLI 不把失败轮次的空文本当成成功回答。
        # if result.get('failed'):
        #     raise RuntimeError(result.get('error') or 'Agent turn failed')
        if not result.get('completed'):
            raise RuntimeError(result.get('error') or result.get('turn_exit_reason') or 'Agent turn did not complete')
        return str(result.get('final_response') or '')

    # [改动][溯源] 原中断能力由基线 run_agent.py:132 的 Mixin 提供；
    # ROADMAP.md:77-79 要求模型请求前及流式处理中读取停止标志。
    def interrupt(self) -> None:
        """设置停止标志；工具执行器据此补齐未完成调用的结果。"""
        self._interrupt_requested = True

    # [改动][溯源] 原 is_interrupted 位于基线 run_agent.py:1102-1105。
    @property
    def is_interrupted(self) -> bool:
        """暴露当前停止标志，供模型循环和工具执行器读取。"""
        return self._interrupt_requested

    # [改动][溯源] 原 _build_system_prompt 转发位于基线 run_agent.py:1107；
    # ROADMAP.md:80-81 仅保留身份、工具指导和时间；片段来自 agent/prompt_builder.py:42-75。
    def _build_system_prompt(self, system_message: str | None = None) -> str:
        """组合 v0.1 的身份、工具规则、当前本地时间及本轮附加系统说明。"""
        ...
        # [改动][溯源] ROADMAP.md:80-82：没有可用搜索时不向模型宣称有工具。
        # parts = [
        #     DEFAULT_AGENT_IDENTITY,
        #     TOOL_USE_ENFORCEMENT_GUIDANCE,
        #     TASK_COMPLETION_GUIDANCE,
    ...
    # [改动][溯源] 原工具名提取位于基线 run_agent.py:1112-1118；
    # 当前 agent/tool_executor.py:1769-1774 同时支持对象和字典格式。
    @staticmethod
    def _tool_call_name(tool_call: Any) -> str:
        """读取对象或字典形式的工具名，以免并发判断漏掉同一调用格式。"""
        function = tool_call.get('function') if isinstance(tool_call, dict) else getattr(tool_call, 'function', None)
        return str(function.get('name') or '') if isinstance(function, dict) else str(getattr(function, 'name', '') or '')

    # [改动][溯源] 原 _execute_tool_calls 位于基线 run_agent.py:1305-1349；
    # ROADMAP.md:73-74 保留独立工具并发，当前仅 web_search 可以进入并发分支。
    def _execute_tool_calls(
        self, assistant_message: Any, messages: list[dict[str, Any]],
        effective_task_id: str, api_call_count: int = 0,
    ) -> None:
        """仅对整批 web_search 并发；其余按序执行并追加配对结果。"""
        ...
        if not tool_calls:
            return
        arguments = (self, assistant_message, messages, effective_task_id, api_call_count)
        self._executing_tools = True
        try:
            if len(tool_calls) > 1 and all(self._tool_call_name(call) == 'web_search' for call in tool_calls):
                execute_tool_calls_concurrent(*arguments)
            else:
                execute_tool_calls_sequential(*arguments)
        finally:
            self._executing_tools = False

    # [改动][溯源] 原 close 位于基线 run_agent.py:949-1034；
    # v0.1 无会话数据库、子 Agent、终端或浏览器资源，当前只负责模型客户端。
    def close(self) -> None:
        """关闭当前模型客户端并清空引用；重复调用不会重复关闭。"""
