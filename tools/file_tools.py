"""Mertina 的最小文件工具：读取、写入、精确替换和文本搜索。

本文件的有效代码只依赖 Python 标准库和 Mertina 注册表；Hermes 原版逐行注释保存在下方，
供以后按需恢复文档提取、远程终端和其他安全策略，不参与当前运行。
"""
...
def _ok(**fields: Any) -> str:
    """把成功结果编码为模型可读取的 JSON 字符串。"""
    return json.dumps({"success": True, **fields}, ensure_ascii=False)


def _file_hash(path: Path) -> str:
    """计算当前文件内容摘要，供整文件覆盖前检查读取基线。"""
    with path.open("rb") as source:
        return hashlib.file_digest(source, "sha256").hexdigest()


def _read_file(args: dict[str, Any], *, agent: Any = None) -> str:
    """按行号读取 UTF-8 文本，并给未读完的内容返回下一页行号。"""
    path = _path(args.get("path"))
    offset = _integer(args.get("offset", 1), "offset", 1, 1_000_000)
    limit = _integer(args.get("limit", 2000), "limit", 1, 2000)
    lines: list[str] = []
    next_offset = None
    chars_read = 0
    with path.open("r", encoding="utf-8", newline="") as source:
        for number, line in enumerate(source, 1):
            ...
            if len(line) > _MAX_READ_CHARS:
                raise ValueError("file contains a line longer than the read limit")
            if len(lines) == limit or chars_read + len(line) > _MAX_READ_CHARS:
                next_offset = number
                break
            clean_line = line.rstrip("\r\n")
            lines.append(f"{number}|{clean_line}")
            chars_read += len(line)
    if offset == 1 and next_offset is None and agent is not None:
        baselines = getattr(agent, "_file_read_baselines", None)
        if baselines is None:
            baselines = agent._file_read_baselines = {}
        baselines[path] = _file_hash(path)
    return _ok(path=str(path), content="\n".join(lines), next_offset=next_offset)


def _write_file(args: dict[str, Any], *, agent: Any = None) -> str:
    """写入完整 UTF-8 内容；覆盖现有文件前要求当前 Agent 已读过该版本。"""
    path = _path(args.get("path"))
    content = args.get("content")
    if not isinstance(content, str):
        raise ValueError("content must be a string")
    if path.exists():
        baselines = getattr(agent, "_file_read_baselines", {})
        if baselines.get(path) != _file_hash(path):
            raise ValueError("read the complete current file before overwriting it")
    ...
    if agent is not None:
        ...
        if baselines is None:
            baselines = agent._file_read_baselines = {}
        baselines[path] = _file_hash(path)
    return _ok(path=str(path), bytes_written=len(content.encode("utf-8")))


def _patch(args: dict[str, Any], **_kwargs: Any) -> str:
    """精确替换一段文本；默认只接受唯一匹配以免改错位置。"""
    path = _path(args.get("path"))
    ...
    if matches == 0 or (matches > 1 and not replace_all):
        raise ValueError(f"expected one match unless replace_all=true; found {matches}")
    updated = original.replace(old, new, -1 if replace_all else 1)
    with path.open("w", encoding="utf-8", newline="") as target:
        target.write(updated)
    return _ok(path=str(path), replacements=matches if replace_all else 1)


def _files_under(path: Path) -> Iterator[Path]:
    """遍历搜索根目录下的文件，并跳过常见缓存与版本控制目录。"""
    if path.is_file():
        yield path
        return
...
def _matching_lines(path: Path, expression: re.Pattern[str]) -> Iterator[dict[str, Any]]:
    """逐行返回文本匹配；无法按 UTF-8 读取的文件视作非文本文件。"""
    try:
        with path.open("r", encoding="utf-8") as source:
            for number, line in enumerate(source, 1):
                if expression.search(line):
                    content = line.rstrip("\r\n")
                    yield {"path": str(path), "line": number,
                           "content": content[:2000], "truncated": len(content) > 2000}
    except (UnicodeError, OSError):
        return


def _search_files(args: dict[str, Any], **_kwargs: Any) -> str:
    """按文件名 glob 或逐行正则搜索，返回带路径和行号的有限结果。"""
    path = _path(args.get("path", "."))
    pattern = args.get("pattern")
    target = args.get("target", "content")
    file_glob = args.get("file_glob")
    limit = _integer(args.get("limit", 50), "limit", 1, 500)
    offset = _integer(args.get("offset", 0), "offset", 0, 1_000_000)
    if not isinstance(pattern, str) or not pattern or target not in {"content", "files"}:
        raise ValueError("pattern and target='content' or 'files' are required")
    if file_glob is not None and not isinstance(file_glob, str):
        raise ValueError("file_glob must be a string")
    if not path.exists():
    ...
    for candidate in _files_under(path):
        if file_glob and not fnmatch.fnmatch(candidate.name, file_glob):
            continue
        if target == "files":
            found = [{"path": str(candidate)}] if fnmatch.fnmatch(candidate.name, pattern) else []
        else:
            found = _matching_lines(candidate, expression)
        for match in found:
            if seen >= offset:
                matches.append(match)
                if len(matches) > limit:
                    return _ok(matches=matches[:limit], has_more=True)
            seen += 1
    return _ok(matches=matches, has_more=False)


def register_file_tools() -> None:
    """登记四个文本文件工具，供对话循环向模型展示并执行。"""
    ...
    registry.register("write_file", {
        "description": "Write complete UTF-8 text; creates parent directories. Read the complete current file before replacing an existing one.",
        "parameters": {"type": "object", "properties": {
            "path": path, "content": {"type": "string"}}, "required": ["path", "content"]},
    }, _write_file)
    registry.register("patch", {
        "description": "Replace exact text in a UTF-8 file. A match must be unique unless replace_all is true.",
        "parameters": {"type": "object", "properties": {
            "path": path, "old_string": {"type": "string"}, "new_string": {"type": "string"},
            "replace_all": {"type": "boolean", "default": False}},
            "required": ["path", "old_string", "new_string"]},
    }, _patch)
    registry.register("search_files", {
        "description": "Search UTF-8 file contents by regex or find files by name glob.",
        "parameters": {"type": "object", "properties": {
            "pattern": {"type": "string"}, "target": {"type": "string", "enum": ["content", "files"]},
            "path": path, "file_glob": {"type": "string"},
            "offset": {"type": "integer", "minimum": 0},
            "limit": {"type": "integer", "minimum": 1, "maximum": 500}}, "required": ["pattern"]},
    }, _search_files)


# === Hermes 原版（已停用；逐行注释保留供溯源）===
# #!/usr/bin/env python3
