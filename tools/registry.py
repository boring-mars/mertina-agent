# 版本 V0.1 变更说明
# [改动][溯源] Hermes 0469740ab33fd02a4f55a6ea11d81df04ea646a5:tools/registry.py；
# ROADMAP.md:73-74,82。上方 Hermes 注册表原位注释保留，文件末尾仅执行显式注册和分发。
# 已注释的功能：插件发现、工具分组、授权与可用性缓存、动态 schema、调试和兼容接口。
# 源代码改动点：保留线程安全的最小 register/get_definitions/dispatch；同步原版注释段的浏览器缓存修正。
# 新增代码：没有新增可执行逻辑；仅补齐原版注释与溯源说明。
...
# [改动][溯源] ROADMAP.md:73-74,82 与 agent/tool_executor.py:1837-1848：
# 本文件只登记工具、给模型提供 schema、按名称分发；上方 Hermes 实现原位注释保留。
import json
...
class ToolRegistry:
    """保存工具 schema 和 handler；只支持当前进程内的显式登记。"""

    def __init__(self) -> None:
        """创建进程内注册表，并保护并发读取和重复登记。"""
    ...
    def register(self, name: str, schema: dict[str, Any], handler: Callable[..., str]) -> None:
        """登记一个函数工具；同一 handler 重复登记是幂等操作。"""
        if not name or not isinstance(schema, dict) or not isinstance(schema.get("parameters"), dict):
            raise ValueError("tool name and object parameters schema are required")
        if not callable(handler):
            raise TypeError("tool handler must be callable")
        with self._lock:
            existing = self._tools.get(name)
            if existing and existing[1] is not handler:
                raise ValueError(f"tool already registered: {name}")
            self._tools[name] = (dict(schema), handler)

    def get_definitions(self, tool_names: set[str]) -> list[dict[str, Any]]:
        """按名称返回 Chat Completions 的函数工具定义。"""
        with self._lock:
            return [
                {"type": "function", "function": {"name": name, **self._tools[name][0]}}
                for name in sorted(tool_names)
                if name in self._tools
            ]

    def dispatch(self, name: str, args: dict[str, Any], **kwargs: Any) -> str:
        """执行已登记工具；错误也变成 JSON 工具结果，保持调用配对。"""
        with self._lock:
            entry = self._tools.get(name)
        if entry is None:
            return tool_error(f"Unknown tool: {name}")
        try:
            result = entry[1](args, **kwargs)
            if not isinstance(result, str):
                return tool_error(f"Tool {name} returned a non-string result")
            return result
        except Exception as exc:
            return tool_error(f"Tool {name} failed: {type(exc).__name__}: {exc}")


def tool_error(message: str) -> str:
    """把可见错误封装为工具协议要求的字符串结果。"""
    return json.dumps({"error": str(message)[:500]}, ensure_ascii=False)
