# 版本 V0.1 变更说明
# [改动][溯源] Hermes 0469740ab33fd02a4f55a6ea11d81df04ea646a5:tools/web_tools.py:268-314,454-503；
# ROADMAP.md:82。保留 web_search 的工具协议、参数入口和注册逻辑。
# 已注释的功能：多后端选择、插件发现、搜索缓存/救援、web_extract、调试和兼容导出；原版见归档段。
# 源代码改动点：以当前 Agent 注入 provider，并按本仓库最小 registry.register 接口登记。
# 新增代码：Brave 作为唯一默认搜索实现；本仓库此前的搜索工具实现也以注释保留。

# === Hermes 原版逐行归档：暂不启用的代码以注释保留 ===
# [溯源] 0469740ab33fd02a4f55a6ea11d81df04ea646a5:tools/web_tools.py
# #!/usr/bin/env python3
...
"""把搜索 provider 包装成模型可调用的 web_search 工具。"""
...
# [改动][溯源] Hermes tools/web_tools.py:454-474 使用 query/limit；Brave Web Search API
# https://api-dashboard.search.brave.com/api-reference/web/search/get 的 count 上限是 20，
# 因此把原版 100 的上限收紧到 20，避免向模型承诺提供不了的条数。
WEB_SEARCH_SCHEMA = {
...
def web_search_tool(query: str, limit: int = 5, *, agent: Any) -> str:
    """使用当前 Agent 的 provider 搜索，返回 Hermes 格式的 JSON 工具结果。"""
    # [改动][溯源] Hermes tools/web_tools.py:268-314 经插件注册表寻找 provider；
    # ROADMAP.md:82 的 v0.1 由 Agent 注入唯一 provider，省去插件依赖。
    ...
    try:
        ...
        limit = 5
    try:
        provider = agent.search_provider or BraveSearchProvider()
        response_data = provider.search(query, limit)
        if not isinstance(response_data, dict) or not isinstance(response_data.get("success"), bool):
            raise TypeError("search provider must return a Hermes search response")
        return json.dumps(response_data, ensure_ascii=False)
    except Exception as exc:
        return json.dumps({"success": False, "error": f"Error searching web: {exc}"}, ensure_ascii=False)


def web_search(args: dict[str, Any], *, agent: Any) -> str:
    """把注册表收到的参数对象转为搜索函数参数。"""
    # [改动][溯源] Hermes tools/web_tools.py:498-503 在 register 的 handler 中读取 args；
    # Mertina 的最小注册表只接受具名 handler，故单独保留这个适配入口。
    return web_search_tool(args.get("query", ""), args.get("limit", 5), agent=agent)


def register_web_search() -> None:
    """在模型请求前登记唯一搜索工具；同一 handler 重复登记安全。"""
    # [改动][溯源] Hermes tools/web_tools.py:498-503 在模块导入时注册；
    # Mertina agent/conversation_loop.py:1857-1858 仅在该 Agent 可用搜索时登记。
    registry.register("web_search", WEB_SEARCH_SCHEMA, web_search)


# === Mertina 改动前实现：count 参数与旧结果结构按原样注释保留 ===
# """[改动][溯源] ROADMAP.md:82：将唯一的 v0.1 工具注册到最小注册表。"""
#
# import json
# from typing import Any
#
...
#
# def web_search(args: dict[str, Any], *, agent: Any) -> str:
#     """调用本 Agent 的搜索 provider，并返回可放入工具消息的 JSON 文本。"""
#     query = args.get("query")
#     if not isinstance(query, str) or not query.strip():
#         return json.dumps({"error": "query must be a non-empty string"}, ensure_ascii=False)
#     provider = agent.search_provider or BraveSearchProvider()
#     results = provider.search(query, args.get("count", 5))
#     return json.dumps({"query": query, "results": results}, ensure_ascii=False)
#
#
# def register_web_search() -> None:
#     """可重复调用；模型请求前确保工具 schema 和 handler 已登记。"""
#     registry.register("web_search", WEB_SEARCH_SCHEMA, web_search)
