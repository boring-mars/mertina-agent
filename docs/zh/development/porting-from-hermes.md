...
从 Hermes 移植的文件保持原路径和原文件名：`agent/turn_tool_round.py` 仍然是
`agent/turn_tool_round.py`。`hermes_*` 前缀替换为 `mertina_*`，即 `hermes_state.py` 变成
`mertina_state.py`，`hermes_cli/` 变成 `mertina_cli/`。

因此不需要逐文件的映射表：一个已移植文件的来源就是它自己的路径。与上游对应文件做 `diff`
得到的是纯语义差异，import 语句两边一字不差。
