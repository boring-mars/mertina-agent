# 版本 V0.1 变更说明
# 当前有效代码只保留最小提示词片段：Agent 身份、工具使用、任务完成和并行调用。
# 注释代码用于保留原始实现，记录后续版本可恢复的功能边界。
#
# 已注释的主要功能（范围对应当前文件中的注释代码）：
# - [121–242] 上下文文件读取超时、注入扫描、Git 根目录查找和 frontmatter 去除；
# - [243–275] Hermes 身份和 Hermes 文档使用指南（仅保留身份片段）；
# - [276–348] 记忆、用户画像、历史会话搜索和技能指导；
# - [349–429] Kanban 任务领取、心跳、评审和多 Agent 协作协议；
# - [430–618] 工具调用、任务完成、并行调用和模型专属执行指导（仅保留通用原则）；
# - [619–704] /steer 中途插话标记、控制帧和桌面悬浮窗提示；
# - [705–928] developer 角色模型名单、渠道消息格式、媒体和定时任务交付提示；
# - [929–1151] Windows/WSL 提示、远程终端探测、容器环境和环境覆盖配置；
# - [1152–1194] 上下文文件字符预算和截断告警；
# - [1195–1566] 技能扫描、条件过滤、分类渲染、内存缓存和磁盘快照；
# - [1567–1831] 内容截断、SOUL.md 和项目规则文件发现与加载；
# - [1832–1852] 上游插件兼容别名及其依赖。
#
# 当前保留常量：
# - DEFAULT_AGENT_IDENTITY：Mertina 的基础身份和回答风格；
# - TOOL_USE_ENFORCEMENT_GUIDANCE：需要工具时实际调用；
# - TASK_COMPLETION_GUIDANCE：依据真实执行结果完成任务；
# - PARALLEL_TOOL_CALL_GUIDANCE：并行执行相互独立的工具调用。
#
# 当前不在本文件生成时间、日期或时区提示；时间层应由后续的系统提示词组装模块按请求生成。
#
...
DEFAULT_AGENT_IDENTITY = (
    # Behavior spec: match response length to the weight of the ask and avoid filler.
